...
const LightNodeContext = createContext<LightNodeContextValue>({
  node: null,
  isLoading: true,
});
...
export const LightNodeProvider: React.FC<LightNodeProviderProps> = ({
  children,
}) => {
  ...
  useEffect(() => {
    const initializeNode = async () => {
      try {
        ...
        toast({ title: "No waku remote peers found" });
      }
    };
    initializeNode().catch((e) =>
      toast({ title: `No waku remote peers found, ${e}` })
    );
  }, []);

  return (
    <LightNodeContext.Provider value={{ node: lightNode, isLoading }}>
      {children}
    </LightNodeContext.Provider>
  );
};
