...
makeSuite("SuperToken Tests", (testEnv: TestEnvironment) => {
    describe("ERC20 and SuperToken Basic Functionality Tests", () => {
        ...
        it("Should support SuperToken without getUnderlyingToken() function implemented", async () => {
            const noGetUnderlyingTokenFactory =
                await hre.ethers.getContractFactory("NoGetUnderlyingToken");
            const noGetUnderlyingToken =
                await noGetUnderlyingTokenFactory.deploy();
            await SuperToken.create({
        ...
        it("Should be able to use downgradeTo", async () => {
            ...
            const bobBalanceBefore =
                await testEnv.wrapperSuperToken.underlyingToken.balanceOf({
                    account: testEnv.bob.address,
            ...
            expect(toBN(aliceBalanceBefore).sub(toBN(amount))).to.equal(
                aliceBalanceAfter.toString()
            );

            // bob new erc20 token balance = prev balance + amount
            expect(toBN(bobBalanceBefore).add(toBN(amount))).to.equal(
