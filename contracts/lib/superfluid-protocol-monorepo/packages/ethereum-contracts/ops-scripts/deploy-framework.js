...
module.exports = eval(`(${S.toString()})({skipArgv: true})`)(async function (
    ...
    const superTokenFactoryNewLogicAddress = await deployContractIf(
        ...
        async () => {
            ...
            try {
                ...
                const poolMemberNFTContract =
                    await PoolMemberNFT.at(poolMemberNFTPAddr);
                const poolAdminNFTLAddr =
                    await poolAdminNFTContract.getCodeAddress();
                const poolMemberNFTLAddr =
        ...
        async () => {
            ...
            if (factoryAddress !== ZERO_ADDRESS) {
                try {
                    const factory =
                        await SuperTokenFactoryLogic.at(factoryAddress);
    ...
    if (
        ...
        console.log(
            `invoking gov.updateContracts(${superfluid.address}, ${superfluidNewLogicAddress}, [${agreementsToUpdate}], ${superTokenFactoryNewLogicAddress})})`
        );
