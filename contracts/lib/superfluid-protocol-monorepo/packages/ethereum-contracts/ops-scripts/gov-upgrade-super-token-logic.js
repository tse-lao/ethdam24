...
module.exports = eval(`(${S.toString()})()`)(async function (
    ...
    skipTokensFile =
        skipTokensFile ||
        process.env.SKIP_OUTPUT_FILE ||
        "./upgrade_skip_tokens.json";
    let skipTokens = [];
    if (fs.existsSync(skipTokensFile)) {
        ...
        console.log(
            `tokens to be skipped: ${JSON.stringify(skipTokens, null, 2)}`
        );
    } else {
        console.warn(
            `File with tokens to be skipped doesn't exist: ${skipTokensFile}`
        );
    }
    ...
    console.log(
        `current canonical super token logic: ${canonicalSuperTokenLogic}`
    );
    ...
    if (tokensToBeUpgraded.length > 0) {
        ...
        if (outputFile !== undefined) {
            fs.writeFileSync(
                outputFile,
                JSON.stringify(tokensToBeUpgraded, null, 2)
            );
            console.log(
                `List of tokens to be upgraded written to ${outputFile}`
            );
        }
        ...
        for (
            let offset = 0;
            offset < tokensToBeUpgraded.length;
            offset += batchSize
        ) {
            ...
            if (!dryRun) {
                ...
                const govAction =
                    superTokenLogic !== undefined
                        ? (gov) =>
                              gov.batchUpdateSuperTokenLogic(
                                  sf.host.address,
                                  batch,
                                  [...new Array(batch.length)].map(
                                      (e) => superTokenLogicAddr
                                  )
                              )
                        : (gov) =>
                              gov.batchUpdateSuperTokenLogic(
                ...
                try {
                    const beaconST = await sf.contracts.ISuperToken.at(
                        batch[0]
                    );
                    const cofAddr = await beaconST.CONSTANT_OUTFLOW_NFT();
                    if (cofAddr === ZERO_ADDRESS) {
                        console.log(
                            "...running upgrade again for NFT initialization..."
                        );
                    ...
                    console.log(
                        `failed to read constantOutflowNFT addr: ${e.toString()}`
                    );
                    console.log(
                        "this is expected if running against a pre-1.6.0 deployment"
                    );
                }
            }
        }
    ...
    return (
        await async.mapLimit(
            ...
            async (superTokenAddress) => {
                const superToken =
                    await sf.contracts.ISuperToken.at(superTokenAddress);
                try {
