...
import { createEventID, initializeEventEntity, tokenHasValidHost } from "../utils";
...
export function handleCustomSuperTokenCreated(event: CustomSuperTokenCreated): void {
    const hostAddress = getHostAddress();
    const hasValidHost = tokenHasValidHost(hostAddress, event.params.token);
    if (!hasValidHost) {
        return;
...
export function handleSuperTokenLogicCreated(event: SuperTokenLogicCreated): void {
    const hostAddress = getHostAddress();
