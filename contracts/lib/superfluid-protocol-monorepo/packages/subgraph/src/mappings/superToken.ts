...
import { BIG_INT_ZERO, createEventID, initializeEventEntity, tokenHasValidHost, ZERO_ADDRESS } from "../utils";
import {
 ...
 * Event Handlers *
 *****************/
export function handleAgreementLiquidatedBy(event: AgreementLiquidatedBy): void {
...
export function handleAgreementLiquidatedV2(event: AgreementLiquidatedV2): void {
    let hostAddress = getHostAddress();
    let hasValidHost = tokenHasValidHost(hostAddress, event.address);
    if (!hasValidHost) {
        return;
...
export function handleTokenUpgraded(event: TokenUpgraded): void {
    ...
    _createAccountTokenSnapshotLogEntity(event, event.params.account, event.address, "TokenUpgraded");
...
export function handleTokenDowngraded(event: TokenDowngraded): void {
    ...
    updateTokenStatsStreamedUntilUpdatedAt(event.address, event.block);
    _createAccountTokenSnapshotLogEntity(event, event.params.account, event.address, "TokenDowngraded");
...
export function handleTransfer(event: Transfer): void {
    ...
    updateAggregateEntitiesTransferData(event.params.from, event.address, event.params.value, event.block);
    ...
    _createAccountTokenSnapshotLogEntity(event, event.params.to, event.address, "Transfer");
    _createAccountTokenSnapshotLogEntity(event, event.params.from, event.address, "Transfer");
...
function updateHOLEntitiesForLiquidation(
    ...
    updateTokenStatsStreamedUntilUpdatedAt(event.address, event.block);
    _createAccountTokenSnapshotLogEntity(event, targetAccount, event.address, eventName);
    _createAccountTokenSnapshotLogEntity(event, liquidatorAccount, event.address, eventName);
    _createAccountTokenSnapshotLogEntity(event, bondAccount, event.address, eventName);
 ...
 ***************************************/
function _createAgreementLiquidatedByEventEntity(event: AgreementLiquidatedBy): void {
    ...
    const streamRevisionId = event.params.id.toHex() + "-" + event.address.toHexString();
...
function _createAgreementLiquidatedV2EventEntity(event: AgreementLiquidatedV2): void {
    ...
    let decoded = ethereum.decode("(uint256,uint256)", event.params.liquidationTypeData) as ethereum.Value;
...
function _createMintedEventEntity(event: Minted): void {
    ...
    initializeEventEntity(ev, event, [event.address, event.params.operator, event.params.to]);
...
function _createTransferEventEntity(event: Transfer): void {
    ...
    initializeEventEntity(ev, event, [event.address, event.params.from, event.params.to]);
...
export function handleApproval(event: Approval): void {
    ...
    const flowOperator = getOrInitFlowOperator(event.block, event.params.spender, event.address, event.params.owner);
