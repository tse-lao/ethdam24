...
export function handleFlowUpdated(event: FlowUpdated): void {
    ...
    const depositResult = cfaContract.try_getFlow(event.params.token, event.params.sender, event.params.receiver);
    ...
    // every time
    const newDeposit = depositResult.reverted ? BigInt.fromI32(0) : depositResult.value.value2; // deposit
    ...
    const userAmountStreamedSinceLastUpdate = oldFlowRate.times(timeSinceLastUpdate);
    const newStreamedUntilLastUpdate = stream.streamedUntilUpdatedAt.plus(userAmountStreamedSinceLastUpdate);
    ...
    if (isDelete) {
        const streamRevision = getOrInitStreamRevision(senderAddress, receiverAddress, tokenAddress);
    ...
    handleStreamPeriodUpdate(event, flowUpdateEvent, oldFlowRate, stream.id, newDeposit);
    ...
    updateATSStreamedAndBalanceUntilUpdatedAt(receiverAddress, tokenAddress, event.block, null);
    ...
    _createAccountTokenSnapshotLogEntity(event, senderAddress, tokenAddress, "FlowUpdated");
    _createAccountTokenSnapshotLogEntity(event, receiverAddress, tokenAddress, "FlowUpdated");
...
export function handleFlowUpdatedExtension(event: FlowUpdatedExtension): void {
    ...
    const flowUpdatedEvent = FlowUpdatedEvent.load(
        "FlowUpdated-" + event.transaction.hash.toHexString() + "-" + previousLogIndex.toString()
    );
    if (flowUpdatedEvent != null) {
        ...
        flowUpdatedEvent.addresses = flowUpdatedEvent.addresses.concat([event.params.flowOperator]);
 ...
 ***************************/
function getFlowActionType(oldFlowRate: BigInt, newFlowRate: BigInt): FlowActionType {
...
function handleStreamPeriodUpdate(
    ...
    const flowActionType = getFlowActionType(previousFlowRate, event.params.flowRate);
    const previousStreamPeriod = StreamPeriod.load(getStreamPeriodID(streamId, streamRevision.periodRevisionIndex));
    switch (flowActionType) {
        case FlowActionType.create:
            startStreamPeriod(eventEntity.id, event, streamRevision, streamId, newDeposit);
            break;
...
function startStreamPeriod(
    ...
    const streamPeriod = new StreamPeriod(getStreamPeriodID(streamId, streamRevision.periodRevisionIndex));
...
export function updateFlowOperatorForFlowUpdated(
    ...
    if (flowUpdatedEvent.type == FlowActionType.create) {
        flowOperator.flowRateAllowanceRemaining = flowOperator.flowRateAllowanceGranted.equals(MAX_FLOW_RATE)
            ? flowOperator.flowRateAllowanceGranted
            : flowOperator.flowRateAllowanceRemaining.minus(flowUpdatedEvent.flowRate);
    }

    if (flowUpdatedEvent.type == FlowActionType.update) {
        const flowRateAllowanceDelta = flowUpdatedEvent.flowRate.minus(flowUpdatedEvent.oldFlowRate);
        flowOperator.flowRateAllowanceRemaining =
            flowOperator.flowRateAllowanceGranted.equals(MAX_FLOW_RATE) || flowRateAllowanceDelta.lt(BIG_INT_ZERO)
                ? flowOperator.flowRateAllowanceGranted
                : flowOperator.flowRateAllowanceRemaining.minus(flowRateAllowanceDelta);
    }
    flowOperator.save();
}
...
function _createFlowUpdatedEntity(
    ...
    initializeEventEntity(ev, event, [event.params.token, event.params.sender, event.params.receiver]);
...
function _createFlowOperatorUpdatedEventEntity(event: FlowOperatorUpdated): FlowOperatorUpdatedEvent {
    const ev = new FlowOperatorUpdatedEvent(createEventID("FlowOperatorUpdated", event));
    initializeEventEntity(ev, event, [event.params.token, event.params.sender, event.params.flowOperator]);

    ev.token = event.params.token;
    ev.sender = event.params.sender;
    ev.permissions = event.params.permissions;
    ev.flowRateAllowance = event.params.flowRateAllowance;
    ev.flowOperator = getFlowOperatorID(event.params.flowOperator, event.params.token, event.params.sender);
