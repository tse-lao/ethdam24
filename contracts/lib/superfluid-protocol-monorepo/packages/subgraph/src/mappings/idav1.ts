...
export function handleIndexCreated(event: IndexCreated): void {
    ...
    tokenStatistic.totalNumberOfIndexes = tokenStatistic.totalNumberOfIndexes + 1;
    ...
    _createAccountTokenSnapshotLogEntity(event, event.params.publisher, event.params.token, eventName);
    _createTokenStatisticLogEntity(event, event.params.token, eventName);
    _createIndexCreatedEventEntity(event, eventName, index.id);
}

export function handleIndexDistributionClaimed(event: IndexDistributionClaimed): void {
    const eventName = "IndexDistributionClaimed";
    const indexId = getIndexID(event.params.publisher, event.params.token, event.params.indexId);
...
export function handleIndexUpdated(event: IndexUpdated): void {
    ...
    const totalUnits = event.params.totalUnitsPending.plus(event.params.totalUnitsApproved);
    const distributionDelta = event.params.newIndexValue.minus(event.params.oldIndexValue).times(totalUnits);
    ...
    const previousTotalAmountDistributed = index.totalAmountDistributedUntilUpdatedAt;
    ...
    index.totalAmountDistributedUntilUpdatedAt = previousTotalAmountDistributed.plus(distributionDelta);
    index.save();
    ...
    if (previousTotalAmountDistributed.equals(BIG_INT_ZERO)) {
        tokenStatistic.totalNumberOfActiveIndexes = tokenStatistic.totalNumberOfActiveIndexes + 1;
    }

    tokenStatistic.totalAmountDistributedUntilUpdatedAt =
        tokenStatistic.totalAmountDistributedUntilUpdatedAt.plus(distributionDelta);
...
export function handleIndexUnitsUpdated(event: IndexUnitsUpdated): void {
    ...
    _createIndexUnitsUpdatedEventEntity(event, eventName, indexId, subscription.units);
}
...
export function handleSubscriptionApproved(event: SubscriptionApproved): void {
    ...
    const balanceDelta = index.indexValue.minus(subscription.indexValueUntilUpdatedAt).times(subscription.units);

    subscription.approved = true;
    subscription.indexValueUntilUpdatedAt = index.indexValue;

    const hasSubscriptionWithUnits = subscriptionWithUnitsExists(subscription.id);
    ...
    if (hasSubscriptionWithUnits) {
        index.totalUnitsApproved = index.totalUnitsApproved.plus(subscription.units);
        index.totalUnitsPending = index.totalUnitsPending.minus(subscription.units);
    ...
    _createAccountTokenSnapshotLogEntity(event, event.params.subscriber, event.params.token, eventName);
    _createTokenStatisticLogEntity(event, event.params.token, eventName);
}

export function handleSubscriptionDistributionClaimed(event: SubscriptionDistributionClaimed): void {
    ...
    const pendingDistribution = subscription.units.times(index.indexValue.minus(subscription.indexValueUntilUpdatedAt));

    subscription.totalAmountReceivedUntilUpdatedAt =
        subscription.totalAmountReceivedUntilUpdatedAt.plus(pendingDistribution);
    subscription.indexValueUntilUpdatedAt = index.indexValue;
    subscription.save();

    _createSubscriptionDistributionClaimedEventEntity(event, eventName, subscription.id);
...
export function handleSubscriptionRevoked(event: SubscriptionRevoked): void {
    ...
    if (subscription.approved) {
        index.totalUnitsApproved = index.totalUnitsApproved.minus(subscription.units);
        index.totalUnitsPending = index.totalUnitsPending.plus(subscription.units);
    }
    subscription.indexValueUntilUpdatedAt = index.indexValue;
    ...
    subscription.totalAmountReceivedUntilUpdatedAt = subscription.totalAmountReceivedUntilUpdatedAt.plus(balanceDelta);
...
export function handleSubscriptionUnitsUpdated(event: SubscriptionUnitsUpdated): void {
    ...
    if (subscription.approved) {
        index.totalUnitsApproved = index.totalUnitsApproved.plus(totalUnitsDelta);
    ...
    if (units.gt(BIG_INT_ZERO) && oldUnits.equals(BIG_INT_ZERO)) {
        index.totalSubscriptionsWithUnits = index.totalSubscriptionsWithUnits + 1;
    ...
    _createSubscriptionUnitsUpdatedEventEntity(event, eventName, subscription.id, oldUnits);
    _createAccountTokenSnapshotLogEntity(event, event.params.publisher, event.params.token, eventName);
    _createAccountTokenSnapshotLogEntity(event, event.params.subscriber, event.params.token, eventName);
    _createTokenStatisticLogEntity(event, event.params.token, eventName);
}
 ...
 ***************************************/
function _createIndexCreatedEventEntity(event: IndexCreated, eventName: string, indexId: string): void {
    const eventId = createEventID(eventName, event);
    const ev = new IndexCreatedEvent(eventId);
    initializeEventEntity(ev, event, [event.params.token, event.params.publisher]);
...
function _createIndexDistributionClaimedEventEntity(
    ...
    initializeEventEntity(ev, event, [event.params.token, event.params.publisher, event.params.subscriber]);
...
function _createIndexUpdatedEventEntity(event: IndexUpdated, eventName: string, indexId: string): void {
    const eventId = createEventID(eventName, event);
...
function _createIndexSubscribedEventEntity(event: IndexSubscribed, eventName: string, indexId: string): void {
    const eventId = createEventID(eventName, event);
...
function _createIndexUnsubscribedEventEntity(event: IndexUnsubscribed, eventName: string, indexId: string): void {
    const eventId = createEventID(eventName, event);
