...
export function handlePoolCreated(event: PoolCreated): void {
    ...
    const tokenStatistic = getOrInitTokenStatistic(event.params.token, event.block);
    tokenStatistic.totalNumberOfPools = tokenStatistic.totalNumberOfPools + 1;

    tokenStatistic.save();

    updateATSStreamedAndBalanceUntilUpdatedAt(event.params.admin, event.params.token, event.block, null);

    _createAccountTokenSnapshotLogEntity(event, event.params.admin, event.params.token, eventName);
...
export function handlePoolConnectionUpdated(event: PoolConnectionUpdated): void {
    // Update Pool Member Entity
    const poolMember = getOrInitPoolMember(event, event.params.pool, event.params.account);
    const previousIsConnected = poolMember.isConnected;
    const memberConnectedStatusUpdated = previousIsConnected !== event.params.connected;
    ...
    if (poolMember.units.gt(BIG_INT_ZERO)) {
        if (memberConnectedStatusUpdated) {
            ...
            if (event.params.connected) {
                pool.totalConnectedUnits = pool.totalConnectedUnits.plus(poolMember.units);
                pool.totalDisconnectedUnits = pool.totalDisconnectedUnits.minus(poolMember.units);
                ...
                pool.totalConnectedUnits = pool.totalConnectedUnits.minus(poolMember.units);
                pool.totalDisconnectedUnits = pool.totalDisconnectedUnits.plus(poolMember.units);
                pool.totalConnectedMembers = pool.totalConnectedMembers - 1;
                pool.totalDisconnectedMembers = pool.totalDisconnectedMembers + 1;
            }
        }
    ...
    updateATSStreamedAndBalanceUntilUpdatedAt(event.params.account, event.params.token, event.block, null);
    ...
    _createAccountTokenSnapshotLogEntity(event, event.params.account, event.params.token, eventName);
...
export function handleBufferAdjusted(event: BufferAdjusted): void {
    ...
    let poolDistributor = getOrInitPoolDistributor(event, event.params.pool, event.params.from);
    poolDistributor = updatePoolDistributorTotalAmountFlowedAndDistributed(event, poolDistributor);
    ...
    const tokenStatistic = getOrInitTokenStatistic(event.params.token, event.block);
    tokenStatistic.totalGDADeposit = tokenStatistic.totalGDADeposit.plus(event.params.bufferDelta);
    tokenStatistic.save();
...
export function handleFlowDistributionUpdated(event: FlowDistributionUpdated): void {
    // Update Pool Distributor
    let poolDistributor = getOrInitPoolDistributor(event, event.params.pool, event.params.distributor);
    poolDistributor = updatePoolDistributorTotalAmountFlowedAndDistributed(event, poolDistributor);
    ...
    const flowRateDelta = event.params.newDistributorToPoolFlowRate.minus(event.params.oldFlowRate);

    const isCreate = event.params.oldFlowRate.equals(BIG_INT_ZERO);
    const isDelete = event.params.newDistributorToPoolFlowRate.equals(BIG_INT_ZERO);
    ...
    // Update ATS
    updateATSStreamedAndBalanceUntilUpdatedAt(event.params.distributor, event.params.token, event.block, null);
    _createAccountTokenSnapshotLogEntity(event, event.params.distributor, event.params.token, eventName);

    // Create Event Entity
    _createFlowDistributionUpdatedEntity(event, poolDistributor.id);
}

export function handleInstantDistributionUpdated(event: InstantDistributionUpdated): void {
    // Update Pool Distributor
    let poolDistributor = getOrInitPoolDistributor(event, event.params.pool, event.params.distributor);
    poolDistributor = updatePoolDistributorTotalAmountFlowedAndDistributed(event, poolDistributor);
    poolDistributor.totalAmountInstantlyDistributedUntilUpdatedAt =
        poolDistributor.totalAmountInstantlyDistributedUntilUpdatedAt.plus(event.params.actualAmount);
    poolDistributor.totalAmountDistributedUntilUpdatedAt = poolDistributor.totalAmountDistributedUntilUpdatedAt.plus(
    ...
    pool.totalAmountInstantlyDistributedUntilUpdatedAt = pool.totalAmountInstantlyDistributedUntilUpdatedAt.plus(
        event.params.actualAmount
    );
    pool.totalAmountDistributedUntilUpdatedAt = pool.totalAmountDistributedUntilUpdatedAt.plus(
        event.params.actualAmount
    );
    pool.save();
    ...
    tokenStatistic.totalAmountDistributedUntilUpdatedAt = tokenStatistic.totalAmountDistributedUntilUpdatedAt.plus(
        event.params.actualAmount
    );
    tokenStatistic.save();
...
function _createPoolConnectionUpdatedEntity(
    ...
    const ev = new PoolConnectionUpdatedEvent(createEventID("PoolConnectionUpdated", event));
    initializeEventEntity(ev, event, [event.params.token, event.params.pool, event.params.account]);
...
function _createBufferAdjustedEntity(event: BufferAdjusted, poolDistributorId: string): BufferAdjustedEvent {
    const ev = new BufferAdjustedEvent(createEventID("BufferAdjusted", event));
    initializeEventEntity(ev, event, [event.params.token, event.params.pool, event.params.from]);
...
function _createInstantDistributionUpdatedEntity(
    ...
    const ev = new InstantDistributionUpdatedEvent(createEventID("InstantDistributionUpdated", event));
    initializeEventEntity(ev, event, [
        event.params.token,
        event.params.pool,
...
function _createFlowDistributionUpdatedEntity(
    ...
    const ev = new FlowDistributionUpdatedEvent(createEventID("FlowDistributionUpdated", event));
    initializeEventEntity(ev, event, [
        event.params.token,
        event.params.pool,
        event.params.distributor,
