...
import { IndexSubscription, Token, TokenStatistic, PoolMember } from "../generated/schema";
...
export function createEventID(eventName: string, event: ethereum.Event): string {
    return eventName + "-" + event.transaction.hash.toHexString() + "-" + event.logIndex.toString();
}
 ...
 */
export function initializeEventEntity(entity: Entity, event: ethereum.Event, addresses: Bytes[]): Entity {
...
export function handleTokenRPCCalls(token: Token, resolverAddress: Address): Token {
...
export function getTokenInfoAndReturn(token: Token): Token {
    ...
    token.underlyingAddress = underlyingAddressResult.reverted ? ZERO_ADDRESS : underlyingAddressResult.value;
 ...
 * @returns
 */
export function tokenHasValidHost(hostAddress: Address, tokenAddress: Address): boolean {
...
export function getStreamRevisionID(senderAddress: Address, receiverAddress: Address, tokenAddress: Address): string {
...
export function getStreamPeriodID(streamId: string, periodRevisionIndex: number): string {
    return streamId + "-" + periodRevisionIndex.toString();
}

export function getFlowOperatorID(flowOperatorAddress: Address, tokenAddress: Address, senderAddress: Address): string {
    return flowOperatorAddress.toHex() + "-" + tokenAddress.toHex() + "-" + senderAddress.toHex();
}
...
export function getIndexID(publisherAddress: Address, tokenAddress: Address, indexId: BigInt): string {
    return publisherAddress.toHex() + "-" + tokenAddress.toHex() + "-" + indexId.toString();
}

export function getPoolMemberID(poolAddress: Address, poolMemberAddress: Address): string {
    return "poolMember-" + poolAddress.toHex() + "-" + poolMemberAddress.toHex();
}

export function getPoolDistributorID(poolAddress: Address, poolDistributorAddress: Address): string {
    return "poolDistributor-" + poolAddress.toHex() + "-" + poolDistributorAddress.toHex();
}

// Get Aggregate ID functions
export function getAccountTokenSnapshotID(accountAddress: Address, tokenAddress: Address): string {
...
export function getActiveStreamsDelta(isCreate: boolean, isDelete: boolean): i32 {
...
export function createLogID(logPrefix: string, accountTokenSnapshotId: string, event: ethereum.Event): string {
    return (
        logPrefix +
        "-" +
