...
import { getHostAddress, getNativeAssetSuperTokenAddress, getResolverAddress } from "./addresses";
 ...
 */
export function getOrInitAccount(accountAddress: Address, block: ethereum.Block): Account {
    ...
    if (account == null) {
        ...
        const appManifestResult = hostContract.try_getAppManifest(accountAddress);
 ...
 */
export function getOrInitSuperToken(event: ethereum.Event, tokenAddress: Address, triggeredByEventName: string): Token {
    ...
    if (token == null) {
        ...
        token.isNativeAssetSuperToken = tokenAddress.equals(nativeAssetSuperTokenAddress);
        ...
        tokenStatistic = getInitialTotalSupplyForSuperToken(tokenStatistic, tokenAddress);
        tokenStatistic.save();

        // Per our TokenStatistic Invariant: whenever we create TokenStatistic, we must create TokenStatisticLog
        _createTokenStatisticLogEntity(event, tokenAddress, triggeredByEventName);

        // If the token has an underlying ERC20, we create a token entity for it.
        if (underlyingAddress.notEqual(ZERO_ADDRESS) && Token.load(token.underlyingAddress.toHex()) == null) {
...
export function getOrInitTokenGovernanceConfig(
    ...
    if (superTokenAddress.equals(ZERO_ADDRESS)) {
        let governanceConfig = TokenGovernanceConfig.load(ZERO_ADDRESS.toHexString());
        if (governanceConfig == null) {
            governanceConfig = new TokenGovernanceConfig(ZERO_ADDRESS.toHexString());
        ...
        let governanceConfig = TokenGovernanceConfig.load(superTokenAddress.toHexString());
        if (governanceConfig == null) {
            governanceConfig = new TokenGovernanceConfig(superTokenAddress.toHexString());
            governanceConfig.createdAtTimestamp = block.timestamp;
            governanceConfig.createdAtBlockNumber = block.number;
            governanceConfig.updatedAtBlockNumber = block.number;
            governanceConfig.updatedAtTimestamp = block.timestamp;
 ...
 */
export function getOrInitToken(tokenAddress: Address, block: ethereum.Block, resolverAddress: Address): void {
    const tokenId = tokenAddress.toHex();
...
export function getOrInitStreamRevision(
    ...
    const streamRevisionId = getStreamRevisionID(senderAddress, recipientAddress, tokenAddress);
...
export function getOrInitStream(event: FlowUpdated): Stream {
    ...
    const streamRevision = getOrInitStreamRevision(event.params.sender, event.params.receiver, event.params.token);
...
export function getOrInitFlowOperator(
    ...
    const flowOperatorId = getFlowOperatorID(flowOperatorAddress, tokenAddress, senderAddress);
    const currentTimestamp = block.timestamp;
    ...
    if (flowOperatorEntity == null) {
        ...
        // https://github.com/superfluid-finance/protocol-monorepo/issues/1397
        const accountTokenSnapshot = getOrInitAccountTokenSnapshot(senderAddress, tokenAddress, block);
...
export function getOrInitSubscription(
    ...
    const subscriptionId = getSubscriptionID(subscriberAddress, publisherAddress, tokenAddress, indexId);
    ...
    if (subscription == null) {
        const index = getOrInitIndex(event, publisherAddress, tokenAddress, indexId, "", triggeredByEventName);
...
export function getOrInitResolverEntry(id: string, target: Address, block: ethereum.Block): ResolverEntry {
...
export function updatePoolTotalAmountFlowedAndDistributed(event: ethereum.Event, pool: Pool): Pool {
    ...
    pool.totalAmountFlowedDistributedUntilUpdatedAt =
        pool.totalAmountFlowedDistributedUntilUpdatedAt.plus(amountFlowedSinceLastUpdate);
    pool.totalAmountDistributedUntilUpdatedAt =
        pool.totalAmountDistributedUntilUpdatedAt.plus(amountFlowedSinceLastUpdate);
...
export function getOrInitPoolDistributor(
    ...
    const poolDistributorID = getPoolDistributorID(poolAddress, poolDistributorAddress);
...
export function updatePoolDistributorTotalAmountFlowedAndDistributed(
    ...
    const timeDelta = event.block.timestamp.minus(poolDistributor.updatedAtTimestamp);
    const amountFlowedSinceLastUpdate = poolDistributor.flowRate.times(timeDelta);

    poolDistributor.updatedAtBlockNumber = event.block.number;
    poolDistributor.updatedAtTimestamp = event.block.timestamp;

    poolDistributor.totalAmountFlowedDistributedUntilUpdatedAt =
        poolDistributor.totalAmountFlowedDistributedUntilUpdatedAt.plus(amountFlowedSinceLastUpdate);
    poolDistributor.totalAmountDistributedUntilUpdatedAt =
        poolDistributor.totalAmountDistributedUntilUpdatedAt.plus(amountFlowedSinceLastUpdate);
...
export function getOrInitAccountTokenSnapshot(
    ...
    if (accountTokenSnapshot == null) {
        ...
        accountTokenSnapshot.totalCFAAmountStreamedInUntilUpdatedAt = BIG_INT_ZERO;
        accountTokenSnapshot.totalGDAAmountStreamedInUntilUpdatedAt = BIG_INT_ZERO;
        accountTokenSnapshot.totalAmountStreamedOutUntilUpdatedAt = BIG_INT_ZERO;
        accountTokenSnapshot.totalCFAAmountStreamedOutUntilUpdatedAt = BIG_INT_ZERO;
        accountTokenSnapshot.totalGDAAmountStreamedOutUntilUpdatedAt = BIG_INT_ZERO;
        accountTokenSnapshot.totalAmountStreamedUntilUpdatedAt = BIG_INT_ZERO;
        accountTokenSnapshot.totalCFAAmountStreamedUntilUpdatedAt = BIG_INT_ZERO;
        accountTokenSnapshot.totalGDAAmountStreamedUntilUpdatedAt = BIG_INT_ZERO;
        accountTokenSnapshot.totalAmountTransferredUntilUpdatedAt = BIG_INT_ZERO;
...
export function _createAccountTokenSnapshotLogEntity(
    ...
    const ats = getOrInitAccountTokenSnapshot(accountAddress, tokenAddress, event.block);
    // Transaction
    const atsLog = new AccountTokenSnapshotLog(createLogID("ATSLog", ats.id, event));
    ...
    atsLog.totalCFAAmountStreamedIn = ats.totalCFAAmountStreamedInUntilUpdatedAt;
    atsLog.totalAmountStreamedOut = ats.totalAmountStreamedOutUntilUpdatedAt;
    atsLog.totalGDAAmountStreamedIn = ats.totalGDAAmountStreamedInUntilUpdatedAt;
    atsLog.totalCFAAmountStreamedOut = ats.totalCFAAmountStreamedOutUntilUpdatedAt;
    atsLog.totalGDAAmountStreamedOut = ats.totalGDAAmountStreamedOutUntilUpdatedAt;
...
export function getOrInitTokenStatistic(tokenAddress: Address, block: ethereum.Block): TokenStatistic {
    const tokenId = tokenAddress.toHex();
...
export function _createTokenStatisticLogEntity(event: ethereum.Event, tokenAddress: Address, eventName: string): void {
    const tokenStatistic = getOrInitTokenStatistic(tokenAddress, event.block);
    const tokenStatisticLog = new TokenStatisticLog(createLogID("TSLog", tokenStatistic.id, event));
    ...
    tokenStatisticLog.totalNumberOfActiveStreams = tokenStatistic.totalNumberOfActiveStreams;
    tokenStatisticLog.totalCFANumberOfActiveStreams = tokenStatistic.totalCFANumberOfActiveStreams;
    tokenStatisticLog.totalGDANumberOfActiveStreams = tokenStatistic.totalGDANumberOfActiveStreams;
    tokenStatisticLog.totalNumberOfClosedStreams = tokenStatistic.totalNumberOfClosedStreams;
    tokenStatisticLog.totalCFANumberOfClosedStreams = tokenStatistic.totalCFANumberOfClosedStreams;
    tokenStatisticLog.totalGDANumberOfClosedStreams = tokenStatistic.totalGDANumberOfClosedStreams;
    tokenStatisticLog.totalNumberOfIndexes = tokenStatistic.totalNumberOfIndexes;
    tokenStatisticLog.totalNumberOfActiveIndexes = tokenStatistic.totalNumberOfActiveIndexes;
    tokenStatisticLog.totalSubscriptionsWithUnits = tokenStatistic.totalSubscriptionsWithUnits;
    tokenStatisticLog.totalApprovedSubscriptions = tokenStatistic.totalApprovedSubscriptions;
    tokenStatisticLog.totalNumberOfPools = tokenStatistic.totalNumberOfPools;
    tokenStatisticLog.totalNumberOfActivePools = tokenStatistic.totalNumberOfActivePools;
    tokenStatisticLog.totalMembershipsWithUnits = tokenStatistic.totalMembershipsWithUnits;
    tokenStatisticLog.totalConnectedMemberships = tokenStatistic.totalConnectedMemberships;
    ...
    tokenStatisticLog.totalAmountStreamed = tokenStatistic.totalAmountStreamedUntilUpdatedAt;
    ...
    tokenStatisticLog.totalAmountTransferred = tokenStatistic.totalAmountTransferredUntilUpdatedAt;
    tokenStatisticLog.totalAmountDistributed = tokenStatistic.totalAmountDistributedUntilUpdatedAt;
 ...
 */
export function updateAccountUpdatedAt(accountAddress: Address, block: ethereum.Block): void {
...
export function updateAggregateDistributionAgreementData(
    ...
    const totalApprovedSubscriptionsDelta = isApproving ? 1 : isRevokingSubscription && subscriptionApproved ? -1 : 0;

    // update ATS Subscription data
    const accountTokenSnapshot = getOrInitAccountTokenSnapshot(accountAddress, tokenAddress, block);

    if (isIDA) {
        accountTokenSnapshot.totalSubscriptionsWithUnits =
            accountTokenSnapshot.totalSubscriptionsWithUnits + totalSubscriptionWithUnitsDelta;
        accountTokenSnapshot.totalApprovedSubscriptions =
            accountTokenSnapshot.totalApprovedSubscriptions + totalApprovedSubscriptionsDelta;
    } else {
        accountTokenSnapshot.totalMembershipsWithUnits =
            accountTokenSnapshot.totalMembershipsWithUnits + totalSubscriptionWithUnitsDelta;
        accountTokenSnapshot.totalConnectedMemberships =
            accountTokenSnapshot.totalConnectedMemberships + totalApprovedSubscriptionsDelta;
    }

    accountTokenSnapshot.isLiquidationEstimateOptimistic =
        accountTokenSnapshot.totalSubscriptionsWithUnits > 0 || accountTokenSnapshot.totalMembershipsWithUnits > 0;
    accountTokenSnapshot.updatedAtTimestamp = block.timestamp;
    accountTokenSnapshot.updatedAtBlockNumber = block.number;
    accountTokenSnapshot.save();
    ...
    if (isIDA) {
        ...
        tokenStatistic.totalApprovedSubscriptions =
            tokenStatistic.totalApprovedSubscriptions + totalApprovedSubscriptionsDelta;
    } else {
        tokenStatistic.totalMembershipsWithUnits =
            tokenStatistic.totalMembershipsWithUnits + totalSubscriptionWithUnitsDelta;
        tokenStatistic.totalConnectedMemberships =
            tokenStatistic.totalConnectedMemberships + totalApprovedSubscriptionsDelta;
    }
...
function updateATSBalanceAndUpdatedAt(
    ...
    const superTokenContract = SuperToken.bind(Address.fromString(accountTokenSnapshot.token));

    if (balanceDelta && accountTokenSnapshot.totalSubscriptionsWithUnits == 0) {
        accountTokenSnapshot.balanceUntilUpdatedAt = accountTokenSnapshot.balanceUntilUpdatedAt.plus(
        ...
        if (!newBalanceResult.reverted) {
            accountTokenSnapshot.balanceUntilUpdatedAt = newBalanceResult.value.value0;
        }
    }
...
export function updateATSStreamedAndBalanceUntilUpdatedAt(
    ...
    let accountTokenSnapshot = getOrInitAccountTokenSnapshot(accountAddress, tokenAddress, block);

    const balanceUntilUpdatedAtBeforeUpdate = accountTokenSnapshot.balanceUntilUpdatedAt;

    //////////////// CFA + GDA streamed amounts ////////////////
    const totalAmountStreamedSinceLastUpdatedAt = getAmountStreamedSinceLastUpdatedAt(
        block.timestamp,
        accountTokenSnapshot.updatedAtTimestamp,
        accountTokenSnapshot.totalNetFlowRate
    );
    const totalAmountStreamedInSinceLastUpdatedAt = getAmountStreamedSinceLastUpdatedAt(
        block.timestamp,
        accountTokenSnapshot.updatedAtTimestamp,
        accountTokenSnapshot.totalInflowRate
    );
    const totalAmountStreamedOutSinceLastUpdatedAt = getAmountStreamedSinceLastUpdatedAt(
        block.timestamp,
        accountTokenSnapshot.updatedAtTimestamp,
    ...
    accountTokenSnapshot.totalAmountStreamedUntilUpdatedAt =
        accountTokenSnapshot.totalAmountStreamedUntilUpdatedAt.plus(totalAmountStreamedSinceLastUpdatedAt);

    // update the totalStreamedUntilUpdatedAt (in)
    accountTokenSnapshot.totalAmountStreamedInUntilUpdatedAt =
        accountTokenSnapshot.totalAmountStreamedInUntilUpdatedAt.plus(totalAmountStreamedInSinceLastUpdatedAt);

    // update the totalStreamedUntilUpdatedAt (out)
    accountTokenSnapshot.totalAmountStreamedOutUntilUpdatedAt =
        accountTokenSnapshot.totalAmountStreamedOutUntilUpdatedAt.plus(totalAmountStreamedOutSinceLastUpdatedAt);
    ...
    accountTokenSnapshot = updateATSBalanceAndUpdatedAt(
        accountTokenSnapshot,
        block,
        balanceDelta ? balanceDelta.plus(totalAmountStreamedSinceLastUpdatedAt) : balanceDelta
    );

    //////////////// CFA streamed amounts ////////////////
    const totalCFAAmountStreamedSinceLastUpdatedAt = getAmountStreamedSinceLastUpdatedAt(
        block.timestamp,
        accountTokenSnapshot.updatedAtTimestamp,
        accountTokenSnapshot.totalCFANetFlowRate
    );
    const totalCFAAmountStreamedInSinceLastUpdatedAt = getAmountStreamedSinceLastUpdatedAt(
        block.timestamp,
        accountTokenSnapshot.updatedAtTimestamp,
        accountTokenSnapshot.totalCFAInflowRate
    );
    const totalCFAAmountStreamedOutSinceLastUpdatedAt = getAmountStreamedSinceLastUpdatedAt(
        block.timestamp,
        accountTokenSnapshot.updatedAtTimestamp,
    ...
    accountTokenSnapshot.totalCFAAmountStreamedUntilUpdatedAt =
        accountTokenSnapshot.totalCFAAmountStreamedUntilUpdatedAt.plus(totalCFAAmountStreamedSinceLastUpdatedAt);

    // update the totalCFAStreamedUntilUpdatedAt (in)
    accountTokenSnapshot.totalCFAAmountStreamedInUntilUpdatedAt =
        accountTokenSnapshot.totalCFAAmountStreamedInUntilUpdatedAt.plus(totalCFAAmountStreamedInSinceLastUpdatedAt);

    // update the totalCFAStreamedUntilUpdatedAt (out)
    accountTokenSnapshot.totalCFAAmountStreamedOutUntilUpdatedAt =
        accountTokenSnapshot.totalCFAAmountStreamedOutUntilUpdatedAt.plus(totalCFAAmountStreamedOutSinceLastUpdatedAt);

    //////////////// GDA streamed amounts ////////////////
    const totalGDAAmountStreamedSinceLastUpdatedAt = getAmountStreamedSinceLastUpdatedAt(
        block.timestamp,
        accountTokenSnapshot.updatedAtTimestamp,
        accountTokenSnapshot.totalGDANetFlowRate
    );
    const totalGDAAmountStreamedInSinceLastUpdatedAt = getAmountStreamedSinceLastUpdatedAt(
        block.timestamp,
        accountTokenSnapshot.updatedAtTimestamp,
        accountTokenSnapshot.totalGDAInflowRate
    );
    const totalGDAAmountStreamedOutSinceLastUpdatedAt = getAmountStreamedSinceLastUpdatedAt(
        block.timestamp,
        accountTokenSnapshot.updatedAtTimestamp,
    ...
    accountTokenSnapshot.totalGDAAmountStreamedUntilUpdatedAt =
        accountTokenSnapshot.totalGDAAmountStreamedUntilUpdatedAt.plus(totalGDAAmountStreamedSinceLastUpdatedAt);

    // update the totalGDAStreamedUntilUpdatedAt (in)
    accountTokenSnapshot.totalGDAAmountStreamedInUntilUpdatedAt =
        accountTokenSnapshot.totalGDAAmountStreamedInUntilUpdatedAt.plus(totalGDAAmountStreamedInSinceLastUpdatedAt);

    // update the totalGDAStreamedUntilUpdatedAt (out)
    accountTokenSnapshot.totalGDAAmountStreamedOutUntilUpdatedAt =
        accountTokenSnapshot.totalGDAAmountStreamedOutUntilUpdatedAt.plus(totalGDAAmountStreamedOutSinceLastUpdatedAt);

    accountTokenSnapshot.save();

    const balanceUntilUpdatedAtAfterUpdate = accountTokenSnapshot.balanceUntilUpdatedAt;

    if (balanceUntilUpdatedAtBeforeUpdate.equals(BIG_INT_ZERO) && balanceUntilUpdatedAtAfterUpdate.gt(BIG_INT_ZERO)) {
        const tokenStatistic = getOrInitTokenStatistic(tokenAddress, block);
        tokenStatistic.totalNumberOfHolders = tokenStatistic.totalNumberOfHolders + 1;
        tokenStatistic.save();
    } else if (
 ...
 * @param block
 */
export function updateTokenStatsStreamedUntilUpdatedAt(tokenAddress: Address, block: ethereum.Block): void {
    const tokenStats = getOrInitTokenStatistic(tokenAddress, block);

    //// CFA + GDA streamed amounts ////
    const amountStreamedSinceLastUpdatedAt = getAmountStreamedSinceLastUpdatedAt(
        block.timestamp,
        tokenStats.updatedAtTimestamp,
        tokenStats.totalOutflowRate
    );
    tokenStats.totalAmountStreamedUntilUpdatedAt = tokenStats.totalAmountStreamedUntilUpdatedAt.plus(
        amountStreamedSinceLastUpdatedAt
    );

    //// CFA streamed amounts ////
    const cfaAmountStreamedSinceLastUpdatedAt = getAmountStreamedSinceLastUpdatedAt(
        block.timestamp,
        tokenStats.updatedAtTimestamp,
        tokenStats.totalCFAOutflowRate
    );
    tokenStats.totalCFAAmountStreamedUntilUpdatedAt = tokenStats.totalCFAAmountStreamedUntilUpdatedAt.plus(
        cfaAmountStreamedSinceLastUpdatedAt
    );

    //// GDA streamed amounts ////
    const gdaAmountStreamedSinceLastUpdatedAt = getAmountStreamedSinceLastUpdatedAt(
        block.timestamp,
        tokenStats.updatedAtTimestamp,
        tokenStats.totalGDAOutflowRate
    );
    tokenStats.totalGDAAmountStreamedUntilUpdatedAt = tokenStats.totalGDAAmountStreamedUntilUpdatedAt.plus(
...
export function updateTokenStatisticStreamData(
    ...
    const tokenStatistic = getOrInitTokenStatistic(tokenAddress, block);
    const totalNumberOfActiveStreamsDelta = getActiveStreamsDelta(isCreate, isDelete);
    const totalNumberOfClosedStreamsDelta = getClosedStreamsDelta(isDelete);

    // the outflow rate should never go below 0.
    tokenStatistic.totalOutflowRate = tokenStatistic.totalOutflowRate.plus(flowRateDelta).lt(BIG_INT_ZERO)
        ? newFlowRate
        : tokenStatistic.totalOutflowRate.plus(flowRateDelta);

    tokenStatistic.totalNumberOfActiveStreams =
        tokenStatistic.totalNumberOfActiveStreams + totalNumberOfActiveStreamsDelta;

    tokenStatistic.totalNumberOfClosedStreams =
        tokenStatistic.totalNumberOfClosedStreams + totalNumberOfClosedStreamsDelta;

    tokenStatistic.totalDeposit = tokenStatistic.totalDeposit.plus(depositDelta);

    if (isCFA) {
        tokenStatistic.totalCFAOutflowRate = tokenStatistic.totalCFAOutflowRate.plus(flowRateDelta).lt(BIG_INT_ZERO)
            ? newFlowRate
            : tokenStatistic.totalCFAOutflowRate.plus(flowRateDelta);

        tokenStatistic.totalCFANumberOfActiveStreams =
            tokenStatistic.totalCFANumberOfActiveStreams + totalNumberOfActiveStreamsDelta;

        tokenStatistic.totalCFANumberOfClosedStreams =
            tokenStatistic.totalCFANumberOfClosedStreams + totalNumberOfClosedStreamsDelta;

        tokenStatistic.totalCFADeposit = tokenStatistic.totalCFADeposit.plus(depositDelta);
    } else {
        tokenStatistic.totalGDAOutflowRate = tokenStatistic.totalGDAOutflowRate.plus(flowRateDelta).lt(BIG_INT_ZERO)
            ? newFlowRate
            : tokenStatistic.totalGDAOutflowRate.plus(flowRateDelta);

        tokenStatistic.totalGDANumberOfActiveStreams =
            tokenStatistic.totalGDANumberOfActiveStreams + totalNumberOfActiveStreamsDelta;

        tokenStatistic.totalGDANumberOfClosedStreams =
            tokenStatistic.totalGDANumberOfClosedStreams + totalNumberOfClosedStreamsDelta;

        tokenStatistic.totalGDADeposit = tokenStatistic.totalGDADeposit.plus(depositDelta);
    }
    tokenStatistic.save();
}
...
export function updateSenderATSStreamData(
    ...
    const totalNumberOfActiveStreamsDelta = getActiveStreamsDelta(isCreate, isDelete);
    const totalNumberOfClosedStreamsDelta = getClosedStreamsDelta(isDelete);
    const senderATS = getOrInitAccountTokenSnapshot(senderAddress, tokenAddress, block);
    senderATS.totalNetFlowRate = senderATS.totalNetFlowRate.minus(flowRateDelta);

    // the outflow rate should never go below 0.
    senderATS.totalOutflowRate = senderATS.totalOutflowRate.plus(flowRateDelta).lt(BIG_INT_ZERO)
        ? newFlowRate
        : senderATS.totalOutflowRate.plus(flowRateDelta);

    senderATS.totalNumberOfActiveStreams = senderATS.totalNumberOfActiveStreams + totalNumberOfActiveStreamsDelta;
    senderATS.activeOutgoingStreamCount = senderATS.activeOutgoingStreamCount + totalNumberOfActiveStreamsDelta;
    senderATS.inactiveOutgoingStreamCount = senderATS.inactiveOutgoingStreamCount + totalNumberOfClosedStreamsDelta;

    senderATS.totalNumberOfClosedStreams = senderATS.totalNumberOfClosedStreams + totalNumberOfClosedStreamsDelta;
    ...
    if (isCFA) {
        senderATS.totalCFANetFlowRate = senderATS.totalCFANetFlowRate.minus(flowRateDelta);

        // the outflow rate should never go below 0.
        senderATS.totalCFAOutflowRate = senderATS.totalCFAOutflowRate.plus(flowRateDelta).lt(BIG_INT_ZERO)
            ? newFlowRate
            : senderATS.totalCFAOutflowRate.plus(flowRateDelta);

        senderATS.totalCFANumberOfActiveStreams =
            senderATS.totalCFANumberOfActiveStreams + totalNumberOfActiveStreamsDelta;
        senderATS.activeCFAOutgoingStreamCount =
            senderATS.activeCFAOutgoingStreamCount + totalNumberOfActiveStreamsDelta;
        senderATS.inactiveCFAOutgoingStreamCount =
            senderATS.inactiveCFAOutgoingStreamCount + totalNumberOfClosedStreamsDelta;

        senderATS.totalCFANumberOfClosedStreams =
            senderATS.totalCFANumberOfClosedStreams + totalNumberOfClosedStreamsDelta;
        senderATS.totalCFADeposit = senderATS.totalCFADeposit.plus(depositDelta);
    } else {
        senderATS.totalGDANetFlowRate = senderATS.totalGDANetFlowRate.minus(flowRateDelta);

        // the outflow rate should never go below 0.
        senderATS.totalGDAOutflowRate = senderATS.totalGDAOutflowRate.plus(flowRateDelta).lt(BIG_INT_ZERO)
            ? newFlowRate
            : senderATS.totalGDAOutflowRate.plus(flowRateDelta);

        senderATS.totalGDANumberOfActiveStreams =
            senderATS.totalGDANumberOfActiveStreams + totalNumberOfActiveStreamsDelta;
        senderATS.activeGDAOutgoingStreamCount =
            senderATS.activeGDAOutgoingStreamCount + totalNumberOfActiveStreamsDelta;
        senderATS.inactiveGDAOutgoingStreamCount =
            senderATS.inactiveGDAOutgoingStreamCount + totalNumberOfClosedStreamsDelta;

        senderATS.totalGDANumberOfClosedStreams =
            senderATS.totalGDANumberOfClosedStreams + totalNumberOfClosedStreamsDelta;
        senderATS.totalGDADeposit = senderATS.totalGDADeposit.plus(depositDelta);
    }
...
export function updateReceiverATSStreamData(
    ...
    const totalNumberOfActiveStreamsDelta = getActiveStreamsDelta(isCreate, isDelete);
    const totalNumberOfClosedStreamsDelta = getClosedStreamsDelta(isDelete);
    const receiverATS = getOrInitAccountTokenSnapshot(receiverAddress, tokenAddress, block);
    receiverATS.totalNetFlowRate = receiverATS.totalNetFlowRate.plus(flowRateDelta);

    // the inflow rate should never go below 0.
    receiverATS.totalInflowRate = receiverATS.totalInflowRate.plus(flowRateDelta).lt(BIG_INT_ZERO)
        ? newFlowRate
        : receiverATS.totalInflowRate.plus(flowRateDelta);

    receiverATS.totalNumberOfActiveStreams = receiverATS.totalNumberOfActiveStreams + totalNumberOfActiveStreamsDelta;
    receiverATS.activeIncomingStreamCount = receiverATS.activeIncomingStreamCount + totalNumberOfActiveStreamsDelta;
    receiverATS.inactiveIncomingStreamCount = receiverATS.inactiveIncomingStreamCount + totalNumberOfClosedStreamsDelta;

    receiverATS.totalNumberOfClosedStreams = receiverATS.totalNumberOfClosedStreams + totalNumberOfClosedStreamsDelta;
    ...
    if (isCFA) {
        receiverATS.totalCFANetFlowRate = receiverATS.totalCFANetFlowRate.plus(flowRateDelta);

        // the inflow rate should never go below 0.
        receiverATS.totalCFAInflowRate = receiverATS.totalCFAInflowRate.plus(flowRateDelta).lt(BIG_INT_ZERO)
            ? newFlowRate
            : receiverATS.totalCFAInflowRate.plus(flowRateDelta);

        receiverATS.totalCFANumberOfActiveStreams =
            receiverATS.totalCFANumberOfActiveStreams + totalNumberOfActiveStreamsDelta;
        receiverATS.activeCFAIncomingStreamCount =
            receiverATS.activeCFAIncomingStreamCount + totalNumberOfActiveStreamsDelta;
        receiverATS.inactiveCFAIncomingStreamCount =
            receiverATS.inactiveCFAIncomingStreamCount + totalNumberOfClosedStreamsDelta;

        receiverATS.totalCFANumberOfClosedStreams =
            receiverATS.totalCFANumberOfClosedStreams + totalNumberOfClosedStreamsDelta;
    } else {
        receiverATS.totalGDANetFlowRate = receiverATS.totalGDANetFlowRate.plus(flowRateDelta);

        // the inflow rate should never go below 0.
        receiverATS.totalGDAInflowRate = receiverATS.totalGDAInflowRate.plus(flowRateDelta).lt(BIG_INT_ZERO)
            ? newFlowRate
            : receiverATS.totalGDAInflowRate.plus(flowRateDelta);

        receiverATS.totalGDANumberOfActiveStreams =
            receiverATS.totalGDANumberOfActiveStreams + totalNumberOfActiveStreamsDelta;
        receiverATS.activeGDAIncomingStreamCount =
            receiverATS.activeGDAIncomingStreamCount + totalNumberOfActiveStreamsDelta;
        receiverATS.inactiveGDAIncomingStreamCount =
            receiverATS.inactiveGDAIncomingStreamCount + totalNumberOfClosedStreamsDelta;

        receiverATS.totalGDANumberOfClosedStreams =
            receiverATS.totalGDANumberOfClosedStreams + totalNumberOfClosedStreamsDelta;
    }
...
export function updateAggregateEntitiesTransferData(
    ...
    const fromAccountTokenSnapshot = getOrInitAccountTokenSnapshot(fromAddress, tokenAddress, block);
    ...
    fromAccountTokenSnapshot.totalAmountTransferredUntilUpdatedAt =
        fromAccountTokenSnapshot.totalAmountTransferredUntilUpdatedAt.plus(value);
