...
import { BondIncreased, ExitRateChanged, NewPIC } from "../../generated/templates/TOGA/TOGA";
import { getAddressEventParam, getBigIntEventParam } from "../converters";

export function createBondIncreasedEvent(token: string, additionalBond: BigInt): BondIncreased {
    const bondIncreasedEvent = changetype<BondIncreased>(newMockEvent());

    const tokenParam = getAddressEventParam("token", token);
    const additionalBondParam = getBigIntEventParam("additionalBond", additionalBond);
...
export function createExitRateChangedEvent(token: string, exitRate: BigInt): ExitRateChanged {
...
export function createNewPICEvent(token: string, pic: string, bond: BigInt, exitRate: BigInt): NewPIC {
