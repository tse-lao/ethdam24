...
import { mockedGetHost, mockedHandleSuperTokenInitRPCCalls } from "../mockedFunctions";
...
describe("SuperTokenFactory Mapper Unit Tests", () => {
    describe("Event Entity Mapping Tests", () => {
        ...
        test("handleSuperTokenCreated() - Should create a new SuperTokenCreatedEvent entity", () => {
            const SuperTokenCreatedEvent = createSuperTokenCreatedEvent(maticXAddress);

            mockedGetHost(maticXAddress);
            mockedHandleSuperTokenInitRPCCalls(maticXAddress, DEFAULT_DECIMALS, ZERO_ADDRESS, maticXName, maticXSymbol);

            handleSuperTokenCreated(SuperTokenCreatedEvent);

            const id = assertEventBaseProperties(SuperTokenCreatedEvent, "SuperTokenCreated");
            assert.fieldEquals("SuperTokenCreatedEvent", id, "token", maticXAddress);
        });

        test("handleCustomSuperTokenCreated() - Should create a new CustomSuperTokenCreatedEvent entity", () => {
            const CustomSuperTokenCreatedEvent = createCustomSuperTokenCreatedEvent(maticXAddress);
            ...
            const id = assertEventBaseProperties(CustomSuperTokenCreatedEvent, "CustomSuperTokenCreated");
            assert.fieldEquals("CustomSuperTokenCreatedEvent", id, "token", maticXAddress);
        });
        ...
        test("handleSuperTokenLogicCreated() - Should create a new SuperTokenLogicCreatedEvent entity", () => {
            ...
            const SuperTokenLogicCreatedEvent = createSuperTokenLogicCreatedEvent(tokenLogic);

            mockedGetHost(superTokenLogicAddress);

            handleSuperTokenLogicCreated(SuperTokenLogicCreatedEvent);

            const id = assertEventBaseProperties(SuperTokenLogicCreatedEvent, "SuperTokenLogicCreated");
            assert.fieldEquals("SuperTokenLogicCreatedEvent", id, "tokenLogic", tokenLogic);
        });
    });
    ...
    describe("Higher Order Entity Mapping Tests", () => {
        ...
        test("handleSuperTokenCreated() - Should create a new Token entity (no underlying)", () => {
            ...
            assert.fieldEquals(
                ...
                maticXAddress,
                "createdAtTimestamp",
                SuperTokenCreatedEvent.block.timestamp.toString()
            );
            assert.fieldEquals(
                "Token",
                maticXAddress,
                "createdAtBlockNumber",
                SuperTokenCreatedEvent.block.number.toString()
            );
        ...
        test("handleSuperTokenCreated() - Should create a new Token entity (with underlying)", () => {
            const SuperTokenCreatedEvent = createSuperTokenCreatedEvent(daiXAddress);
            ...
            mockedHandleSuperTokenInitRPCCalls(daiAddress, DEFAULT_DECIMALS, ZERO_ADDRESS, daiName, daiSymbol);
