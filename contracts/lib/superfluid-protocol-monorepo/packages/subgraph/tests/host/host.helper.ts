...
import { getAddressEventParam, getBytesEventParam, getBigIntEventParam } from "../converters";

export function createGovernanceReplacedEvent(oldGovernance: string, newGovernance: string): GovernanceReplaced {
    const newGovernanceReplacedEvent = changetype<GovernanceReplaced>(newMockEvent());
...
export function createAgreementClassRegisteredEvent(agreementType: Bytes, code: string): AgreementClassRegistered {
    const newAgreementClassRegisteredEvent = changetype<AgreementClassRegistered>(newMockEvent());

    newAgreementClassRegisteredEvent.parameters = new Array();
    const agreementTypeParam = getBytesEventParam("agreementType", agreementType);
...
export function createSuperTokenFactoryUpdatedEvent(newFactory: string): SuperTokenFactoryUpdated {
    const newSuperTokenFactoryUpdatedEvent = changetype<SuperTokenFactoryUpdated>(newMockEvent());
...
export function createSuperTokenLogicUpdatedEvent(token: string, code: string): SuperTokenLogicUpdated {
    const newSuperTokenLogicUpdatedEvent = changetype<SuperTokenLogicUpdated>(newMockEvent());
