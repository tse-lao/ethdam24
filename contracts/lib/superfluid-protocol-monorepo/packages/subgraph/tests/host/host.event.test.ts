...
describe("Host Event Entity Unit Tests", () => {
    ...
    test("handleGovernanceReplaced() - Should create a new GovernanceReplacedEvent entity", () => {
        const oldGov = alice;
        const newGov = bob;
        const governanceReplacedEvent = createGovernanceReplacedEvent(oldGov, newGov);

        handleGovernanceReplaced(governanceReplacedEvent);

        const id = assertEventBaseProperties(governanceReplacedEvent, "GovernanceReplaced");
        assert.fieldEquals("GovernanceReplacedEvent", id, "oldGovernance", oldGov);
        assert.fieldEquals("GovernanceReplacedEvent", id, "newGovernance", newGov);
    });

    test("handleAgreementClassRegistered() - Should create a new AgreementClassRegisteredEvent entity", () => {
        const code = alice;
        const bytesString = ByteArray.fromUTF8("org.superfluid-finance.contracts.Superfluid.implementation");
        const agreementType = Bytes.fromByteArray(crypto.keccak256(bytesString));
        const agreementClassRegisteredEvent = createAgreementClassRegisteredEvent(agreementType, code);

        handleAgreementClassRegistered(agreementClassRegisteredEvent);

        const id = assertEventBaseProperties(agreementClassRegisteredEvent, "AgreementClassRegistered");
        assert.fieldEquals("AgreementClassRegisteredEvent", id, "agreementType", agreementType.toHexString());
        assert.fieldEquals("AgreementClassRegisteredEvent", id, "code", code);
    });

    test("handleSuperTokenFactoryUpdated() - Should create a new SuperTokenFactoryUpdatedEvent entity", () => {
        const newFactory = alice;
        const superTokenFactoryUpdatedEvent = createSuperTokenFactoryUpdatedEvent(newFactory);

        handleSuperTokenFactoryUpdated(superTokenFactoryUpdatedEvent);

        const id = assertEventBaseProperties(superTokenFactoryUpdatedEvent, "SuperTokenFactoryUpdated");
        assert.fieldEquals("SuperTokenFactoryUpdatedEvent", id, "newFactory", newFactory);
    });

    test("handleSuperTokenLogicUpdated() - Should create a new SuperTokenLogicUpdatedEvent entity", () => {
        const token = alice;
        const code = token;
        const superTokenLogicUpdatedEvent = createSuperTokenLogicUpdatedEvent(token, code);

        handleSuperTokenLogicUpdated(superTokenLogicUpdatedEvent);

        const id = assertEventBaseProperties(superTokenLogicUpdatedEvent, "SuperTokenLogicUpdated");
    ...
    test("handleAppRegistered() - Should create a new AppRegisteredEvent entity", () => {
        ...
        const id = assertEventBaseProperties(appRegisteredEvent, "AppRegistered");
