...
export const subgraphRequest = async <T>(
    ...
    try {
        ...
        throw new Error(`Failed call to subgraph with query ${query} and error ${err}`);
    }
};
...
async function getAllResults<T>(query: string, endpoint: string, resultsPerPage: number, variables: any): Promise<T[]> {
    const initialResults = await subgraphRequest<{ response: T[] }>(query, endpoint, variables);
    ...
    const data = [
        ...
        ...((await getAllResults(query, endpoint, resultsPerPage, newVariables)) as T[]),
    ];
    return data;
}

const liquidatedByQuery = gql`
    query getLiquidatedByEvents($lte_timestamp: Int!, $gte_timestamp: Int!, $first: Int!, $block: Int!) {
        response: agreementLiquidatedByEvents(
            block: { number: $block }
            where: { timestamp_lte: $lte_timestamp, timestamp_gte: $gte_timestamp }
            first: $first
            orderBy: timestamp
            orderDirection: asc
        ) {
...
const tokensQuery = gql`
    query getTokens($id_in: [ID!], $block: Int!) {
        response: tokens(block: { number: $block }, where: { underlyingToken_not: null, id_in: $id_in }) {
            id
            underlyingToken {
                id
                name
...
const liquidatedStreamsQuery = gql`
    query getStreams($sender_in: [String!], $createdAtTimestamp_gte: BigInt!, $first: Int!, $block: Int!) {
        response: streams(
            first: $first
            block: { number: $block }
            where: { sender_in: $sender_in, createdAtTimestamp_gte: $createdAtTimestamp_gte, currentFlowRate: 0 }
        ) {
            id
            createdAtTimestamp
            sender {
...
const getLiquidatedStreams = async (chainId: number, providerEndpoint: string, startTime: number, endTime: number) => {
    ...
    const recentBlock = await getMostRecentIndexedBlockNumber(chainIdData.subgraphAPIEndpoint);
    const provider = new ethers.providers.JsonRpcProvider(providerEndpoint);
    const block = await provider.getBlock(recentBlock);

    if (endTime > block.timestamp) {
        console.error("The end time is greater than the most recently indexed block.");
        return;
    }
    ...
    const agreementLiquidatedByEvents = await getAllResults<IAgreementLiquidatedByEvent>(
        liquidatedByQuery,
    ...
    const uniqueLiquidatedByEvents = _.uniqBy(agreementLiquidatedByEvents, (x) => x.id);
    console.log("Number of unique AgreementLiquidatedBy events:", uniqueLiquidatedByEvents.length);
    ...
    const liquidatedTokens = _.uniqBy(uniqueLiquidatedByEvents, (x) => x.token).map((x) => x.token);
    ...
    const superTokenSymbolToBoolDict = superTokensResponse.response.reduce(
        (acc, obj) => {
            return obj.underlyingToken ? { ...acc, [obj.underlyingToken.symbol.toLowerCase()]: true } : { ...acc };
        },
    ...
    const superTokenSymbolToIdDict = superTokensResponse.response.reduce(
        (acc, obj) => {
            ...
            return underlyingToken ? { ...acc, [underlyingToken.symbol.toLowerCase()]: id } : { ...acc };
        },
        {} as { [symbol: string]: string }
    );

    // filter coingecko coin list to only include liquidated tokens
    const filteredCoinList = coingeckoCoinList.data.filter((x) => superTokenSymbolToBoolDict[x.symbol.toLowerCase()]);
    ...
    const filteredAgreementLiquidatedByEvents = uniqueLiquidatedByEvents.filter(
        (x) => Number(ethers.utils.formatUnits(x.rewardAmount)) * Number(tokenPrices[x.token]) >= MIN_DOLLAR_AMOUNT
    );

    const filteredPenaltyAccountToTxnHashDict = filteredAgreementLiquidatedByEvents.reduce(
        (acc, obj) => {
            const { transactionHash, penaltyAccount, token } = obj;
            return {
                ...acc,
                [penaltyAccount + "-" + token]: [...(acc[penaltyAccount + "-" + token] || []), transactionHash],
            };
        },
    ...
    const liquidatedIndividuals = filteredAgreementLiquidatedByEvents.map((x) => x.penaltyAccount);
    const uniqueLiquidatedIndividuals = _.uniq(liquidatedIndividuals);

    console.log("Unique Liquidated Individuals Length:", uniqueLiquidatedIndividuals.length);
    ...
    console.log("Unique Possible Liquidated Streams Length:", uniquePossibleLiquidatedStreams.length);
    ...
    const weekOldLikelyLiquidatedStreams = uniquePossibleLiquidatedStreams
        ...
        .filter((x) => Number(x.flowUpdatedEvents[0].timestamp) - Number(x.createdAtTimestamp) <= WEEK_IN_SECS) // the stream should be less than a week old
        .map((x) => ({
            ...
            totalAmountStreamedUntilTimestamp: x.flowUpdatedEvents[0].totalAmountStreamedUntilTimestamp,
            transactionHash: x.flowUpdatedEvents[0].transactionHash,
            terminatedAtTimestamp: x.flowUpdatedEvents[0].timestamp,
        }))
        .filter((x) =>
            (filteredPenaltyAccountToTxnHashDict[x.sender + "-" + x.token] || []).includes(x.transactionHash)
        );
    console.log(
