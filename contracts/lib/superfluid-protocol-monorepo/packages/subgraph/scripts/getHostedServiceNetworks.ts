...
function main() {
    const networks = JSON.stringify(
        metadata.networks.filter((x) => x.subgraphV1.hostedEndpoint != null).map((x) => x.name)
    );

    const writeToDir = __dirname.split("subgraph")[0] + "subgraph/hosted-service-networks.json";
