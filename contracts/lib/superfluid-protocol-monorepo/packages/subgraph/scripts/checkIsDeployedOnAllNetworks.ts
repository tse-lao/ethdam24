...
const subgraphRequest = async <T>(query: string): Promise<T> => {
    try {
        const response = await request<T>("https://api.thegraph.com/index-node/graphql", query);
...
const executeIndexingStatusQuery = async (networkName: string) => {
    const subgraphName = "superfluid-finance/protocol-" + process.env.SUBGRAPH_RELEASE_TAG + "-" + networkName;
