...
async function main() {
    ...
    const currentBlockNumber = await getMostRecentIndexedBlockNumber(chainIdData.subgraphAPIEndpoint);
    const queryHelper = new QueryHelper(chainIdData.subgraphAPIEndpoint, currentBlockNumber, MAX_RESULTS_PER_PAGE);
    const block = await ethers.provider.getBlock(currentBlockNumber);
    const currentTimestamp = block.timestamp;
    console.log("\nExecuting Subgraph Data Integrity Test on " + chainIdData.name + " network.");
    console.log("Current block number used to query: ", currentBlockNumber);
    console.log("Current timestamp: ", currentTimestamp);
    const addresses = chainIdData.addresses;

    const cfaV1 = IConstantFlowAgreementV1__factory.connect(addresses.cfaAddress, ethers.provider);
    const idaV1 = IInstantDistributionAgreementV1__factory.connect(addresses.idaAddress, ethers.provider);
    ...
    await Promise.all(
        keys(onChainCFAEvents).map(async (x) => {
            ...
            const events = await cfaV1.queryFilter(eventsFilter, addresses.hostStartBlock);
            const groupedEvents = _.groupBy(events, (x) => x.transactionHash.toLowerCase() + "-" + x.logIndex);
    ...
    await Promise.all(
        keys(onChainIDAEvents).map(async (x) => {
            ...
            const events = await idaV1.queryFilter(eventsFilter, addresses.hostStartBlock);
            const groupedEvents = _.groupBy(events, (x) => x.transactionHash.toLowerCase() + "-" + x.logIndex);
    ...
    const getEventErrorArray = async <T>(onChainEvents: T, eventToQueryMap: Map<keyof T, string>) => {
    ...
    const cfaErrorArray = await getEventErrorArray(onChainCFAEvents, cfaEventToQueryMap);

    console.log("\nIDA Events Validation Starting...");
    const idaErrorArray = await getEventErrorArray(onChainIDAEvents, idaEventToQueryMap);

    const eventErrorsSum = cfaErrorArray.concat(idaErrorArray).reduce((x, y) => x + y, 0);
    ...
    const accountTokenSnapshots = await queryHelper.getAllResults<DataIntegrityAccountTokenSnapshot>({
    ...
    const subscriptions = await queryHelper.getAllResults<DataIntegritySubscription>({
    ...
    // Gets all subscriptions ever created
    const tokenStatistics = await queryHelper.getAllResults<DataIntegrityTokenStatistic>({
    ...
    const uniqueStreams = _.uniqBy(streams, (x) => x.createdAtTimestamp + x.sender.id + x.receiver.id + x.token.id);
    console.log(`There are ${uniqueStreams.length} unique streams out of ${streams.length} total queried streams.`);

    const uniqueAccountTokenSnapshots = _.uniqBy(accountTokenSnapshots, (x) => x.id);
    console.log(
    ...
    for (let i = 0; i < chunkedUniqueStreams.length; i++) {
        await Promise.all(
            chunkedUniqueStreams[i].map(async (x) => {
                const stream = x;
                try {
                    const { timestamp, flowRate, deposit } = await cfaV1.getFlow(
                    ...
                    const updatedAtShouldMatch = timestamp.eq(toBN(stream.updatedAtTimestamp));
                    ...
                    if (!updatedAtShouldMatch || !flowRateShouldMatch || !depositShouldMatch) {
                        flowDataErrors++;
                        throw new Error(
                            "Values don't match. \n Subgraph Stream: " +
    ...
    console.log("Validating " + uniqueAccountTokenSnapshots.length + " account token snapshot net flow rates.");
    const chunkedUniqueAccountTokenSnapshots = chunkData(uniqueAccountTokenSnapshots, DEFAULT_CHUNK_LENGTH);
    ...
    for (let i = 0; i < chunkedUniqueAccountTokenSnapshots.length; i++) {
        await Promise.all(
            chunkedUniqueAccountTokenSnapshots[i].map(async (x) => {
                try {
                    const netFlowRate = await cfaV1.getNetFlow(x.token.id, x.account.id, {
                        blockTag: currentBlockNumber,
                    });
                    const netFlowRateShouldMatch = netFlowRate.eq(toBN(x.totalNetFlowRate));
    ...
    printTestOutcome(netFlowErrors === 0, "Success (A.1.b): Net Flow matching", "Failure (A.1.b): Net Flow mismatch");
    ...
    console.log("Account Level RTB Invariant: User RTB === Subgraph calculated balance Test Starting... (A.0)");
    const uniqueTokens = _.uniqBy(uniqueAccountTokenSnapshots, (x) => x.token.id).map((x) => x.token.id);
    let tokenContracts: { [tokenAddress: string]: ISuperToken } = {};
    let realtimeBalanceErrors = 0;
    for (let i = 0; i < uniqueTokens.length; i++) {
        tokenContracts[uniqueTokens[i]] = ISuperToken__factory.connect(uniqueTokens[i], ethers.provider);
    }
    ...
    for (let i = 0; i < chunkedUniqueAccountTokenSnapshots.length; i++) {
        await Promise.all(
            ...
            chunkedUniqueAccountTokenSnapshots[i].map(async (x, _j) => {
                try {
                    ...
                    const [realtimeBalance] = await tokenContract.realtimeBalanceOfNow(x.account.id, {
                        blockTag: currentBlockNumber,
                    });
                    ...
                    const userIndexSubscriptions = uniqueSubscriptions.filter(
                        (z) => z.index.token.id === x.token.id && z.subscriber.id === x.account.id
                    );
                    ...
                    const calculatedAvailableBalance = calculateAvailableBalance({
                        ...
                        indexSubscriptions: userIndexSubscriptions as unknown as IIndexSubscription[],
                    });
        ...
        printProgress(i, chunkedUniqueAccountTokenSnapshots.length, "User RTB === calculated balance");
    }
        ...
        await Promise.all(
            chunkedUniqueIndexes[i].map(async (x) => {
                ...
                try {
                    ...
                    const [exist, indexValue, totalUnitsApproved, totalUnitsPending] = await idaV1.getIndex(
                        superToken,
                        publisher,
                        indexId,
                        {
                    ...
                    const totalUnitsApprovedShouldMatch = toBN(index.totalUnitsApproved).eq(totalUnitsApproved);
                    const totalUnitsPendingShouldMatch = toBN(index.totalUnitsPending).eq(totalUnitsPending);
                    const compareIndex = {
                    ...
                    if (!indexValueShouldMatch || !totalUnitsApprovedShouldMatch || !totalUnitsPendingShouldMatch) {
        ...
        await Promise.all(
            chunkedUniqueSubscriptions[i].map(async (x) => {
                ...
                try {
                    ...
                    const publisher = ethers.utils.getAddress(subscription.index.publisher.id);
                    const subscriber = ethers.utils.getAddress(subscription.subscriber.id);
                    const indexId = Number(subscription.index.indexId);
                    const [exist, approved, units, pendingDistribution] = await idaV1.getSubscription(
                        superToken,
                        publisher,
                        indexId,
                        subscriber,
                    ...
                    const expectedPendingDistribution = subscription.approved
                        ...
                        : toBN(subscription.units).mul(
                              toBN(subscription.index.indexValue).sub(toBN(subscription.indexValueUntilUpdatedAt))
                          );
                    ...
                    const pendingDistributionShouldMatch = expectedPendingDistribution.eq(pendingDistribution);
                    ...
                    if (!approvedShouldMatch || !unitsShouldMatch || !pendingDistributionShouldMatch) {
                        subscriptionDataErrors++;
                        throw new Error(
    ...
    for (let i = 0; i < chunkedUniqueTokenStatistics.length; i++) {
        await Promise.all(
            chunkedUniqueTokenStatistics[i].map(async (x) => {
                try {
                    const superTokenContract = ISuperToken__factory.connect(x.id, ethers.provider);
                    const tokenContract = ISuperToken__factory.connect(x.token.underlyingAddress, ethers.provider);
                    const totalSupply = await superTokenContract.totalSupply();
                    let aum = await tokenContract.balanceOf(x.id);
                    const superTokenDecimals = await superTokenContract.decimals();
                    const tokenDecimals = await tokenContract.decimals();
                    const decimalsDifference = superTokenDecimals - tokenDecimals;
                    const formattedAUM =
                        decimalsDifference > 0 ? ethers.utils.parseUnits(aum.toString(), decimalsDifference) : aum;

                    const getTokenNameAndLog = async () => {
                        const name = await tokenContract.name();
                        console.log(`TOKEN (${x.token.underlyingAddress}): ${name}`);
                    };
