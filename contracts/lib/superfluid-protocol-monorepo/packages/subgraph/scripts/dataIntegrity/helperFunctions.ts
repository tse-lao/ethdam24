...
export const chunkData = <T>(promises: T[], chunkLength: number) => {
    const chunksLength = promises.length < chunkLength ? promises.length : Math.ceil(promises.length / chunkLength);
...
export const validateEvents = <T extends TypedEvent, K>(
    eventName: string,
    onChainData: Record<string, { events: T[]; groupedEvents: _.Dictionary<T[]> }>,
    subgraphEvents: K[]
) => {
    let errors = 0;
    ...
    for (let i = 0; i < subgraphEvents.length; i++) {
        ...
        for (let j = 0; j < keys.length; j++) {
            ...
            if (currentOnChainEvent.args[keys[j]] == null || currentOnChainEvent.args[keys[j]] == undefined) {
                continue;
            }

            // on-chain data is type string
            if (typeof currentOnChainEvent.args[keys[j]] === "string") {
                if (currentOnChainEvent.args[keys[j]].toLowerCase() !== currentSubgraphEvent[keys[j]]) {
                    console.log(
                        `${keys[j]} - Subgraph: ${
                            currentSubgraphEvent[keys[j]]
                        } \n On-chain data: ${currentOnChainEvent.args[keys[j]].toLowerCase()}`
                    );
                    errors++;
                ...
                if (toBN(currentOnChainEvent.args[keys[j]]).eq(toBN(currentSubgraphEvent[keys[j]])) === false) {
                    console.log(
                        `${keys[j]} - Subgraph: ${
                            currentSubgraphEvent[keys[j]]
                        } \n On-chain data: ${currentOnChainEvent.args[keys[j]].toString()}`
                    );
                    errors++;
                    throw new Error(`${eventName} Event is not the same.`);
                }
                ...
                if (currentOnChainEvent.args[keys[j]].eq(toBN(currentSubgraphEvent[keys[j]])) === false) {
                    console.log(
                        `${keys[j]} - Subgraph: ${
                            currentSubgraphEvent[keys[j]]
                        } \n On-chain data: ${currentOnChainEvent.args[keys[j]].toString()}`
...
export const querySubgraphAndValidateEvents = async ({
    ...
    return validateEvents(eventName, onChainEvents as any, uniqueSubgraphEvents);
};
...
export class QueryHelper {
    ...
    constructor(endpoint: string, currentBlockNumber: number, resultsPerPage: number) {
    ...
    getAllResults = async <T extends BaseEntity>({
        ...
        console.log(counter * this.resultsPerPage + initialResults.response.length + " responses queried.");
        counter++;
        ...
        let newTimestamp = isEvent
            ...
            : isUpdatedAt
              ? initialResults.response[initialResults.response.length - 1].updatedAtTimestamp
              : initialResults.response[initialResults.response.length - 1].createdAtTimestamp;
...
export const printTestOutcome = (success: boolean, successText: string, failureText: string) => {
    if (success) {
