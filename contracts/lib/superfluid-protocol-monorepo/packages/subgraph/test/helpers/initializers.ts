...
export const getOrInitSubscription = (
    ...
    if (existingSubscription == null) {
        const [subscriber, publisher, token, indexId] = subscriptionId.split("-");
 ...
 * @returns
 */
export function getOrInitializeDataForFlowUpdated(initTestData: IFlowUpdatedInitTestData) {
    const data = initTestData.data;
    const localData = initTestData.data.localData;
    const tokenAddress = data.superToken.address;

    const revisionIndexId = getRevisionIndexId(data.sender, data.receiver, tokenAddress);
    ...
    const currentRevisionIndex = getOrInitRevisionIndex(localData.revisionIndexes, revisionIndexId);
    const currentPeriodRevisionIndex = getOrInitPeriodRevisionIndex(localData.periodRevisionIndexes, revisionIndexId);
    const streamId = getStreamId(data.sender, data.receiver, tokenAddress, currentRevisionIndex.toString());
...
export function getOrInitializeDataForFlowOperatorUpdated(testData: IFlowOperatorUpdatedInitTestData) {
 ...
 * @returns
 */
export function getOrInitializeDataForIDA(testData: IInstantDistributionTestData) {
    const {
        accountTokenSnapshots,
        indexes,
        indexId,
    ...
    const subscriptionId = getSubscriptionId(subscriberAddress, publisher, token, indexId);
    const indexEntityId = getIndexId(publisher, token, indexId);
    const currentIndex = getOrInitIndex(indexes, indexEntityId, updatedAtBlockNumber, updatedAtTimestamp);
