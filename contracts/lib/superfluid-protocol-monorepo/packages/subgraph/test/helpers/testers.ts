...
import { getFlowOperatorUpdatedEvents, getFlowUpdatedEvents, getTransferEvents } from "../queries/eventQueries";
...
export async function testFlowUpdated(data: ITestModifyFlowData) {
    ...
    const streamedAmountSinceUpdatedAt = toBN(initData.pastStreamData.oldFlowRate).mul(
        toBN(lastUpdatedAtTimestamp).sub(toBN(initData.pastStreamData.updatedAtTimestamp))
    );
    ...
    const streamedAmountUntilTimestamp = toBN(initData.pastStreamData.streamedUntilUpdatedAt).add(
        streamedAmountSinceUpdatedAt
    );

    const flowUpdatedEvent = await fetchEventAndValidate<IFlowUpdatedEvent, IExpectedFlowUpdateEvent>(
        txnResponse,
        {
            ...
            totalAmountStreamedUntilTimestamp: streamedAmountUntilTimestamp.toString(),
    ...
    if (data.liquidator) {
        const transferEvent = await fetchEventAndEnsureExistence<ITransferEvent>(
            getTransferEvents,
            txnResponse.hash,
            "TransferEvents"
        );
        ...
        expectedData.updatedSenderATS = {
            ...
            totalAmountTransferredUntilUpdatedAt: toBN(
                expectedData.updatedSenderATS.totalAmountTransferredUntilUpdatedAt
            )
...
export async function testUpdateFlowOperatorPermissions(data: ITestUpdateFlowOperatorData) {
    ...
    const expectedPermissions = data.isFullControl ? FULL_CONTROL : data.isFullControlRevoke ? 0 : data.permissions;
    ...
    const senderATSFlowOperators = senderATS.flowOperators.map((x) => x.id).includes(flowOperatorId)
...
export async function testModifyIDA(data: ITestModifyIDAData) {
    ...
    const { accountTokenSnapshots, indexes, subscriptions, tokenStatistics } = localData;
    ...
    if (eventType === IDAEventType.IndexUpdated) {
        if (amountOrIndexValue == null) {
            throw new Error("amountOrIndexValue must be passed for IndexUpdated.");
        }
        ...
        totalUnits = toBN(initData.currentIndex.totalUnitsApproved).add(toBN(initData.currentIndex.totalUnitsPending));

        const indexDelta = totalUnits.gt(toBN(0)) ? toBN(amountOrIndexValue.toString()).div(totalUnits) : toBN(0);
        newIndexValue =
            isDistribute === true
    ...
    let distributionDelta = toBN(initData.currentSubscription.units).mul(
        toBN(initData.currentIndex.indexValue).sub(toBN(initData.currentSubscription.indexValueUntilUpdatedAt))
    );
    ...
    const isEventEmitted =
        eventType !== IDAEventType.SubscriptionDistributionClaimed || pendingDistribution.gt(toBN(0));
    let events: IIDAEvents = {};

    if (isEventEmitted) {
        events = await fetchIDAEventsAndValidate(eventType, txnResponse, baseParams, extraEventData);
        ...
        if (
            ...
            const otherEventType = subscriptionEventTypeToIndexEventType.get(eventType);
            if (!otherEventType) {
            ...
            events = {
                ...events,
                ...(await fetchIDAEventsAndValidate(otherEventType, txnResponse, baseParams, extraEventData)),
            };
        }
    }
    ...
    const { updatedIndex, updatedPublisherATS, updatedSubscription, updatedSubscriberATS, updatedTokenStats } =
...
async function executeIDATransactionByTypeAndWaitForIndexer(
    ...
    if (type === IDAEventType.IndexCreated) {
        ...
        if (amountOrIndexValue == null || isDistribute == null) {
            throw new Error("You must pass isDistribute and amountOrIndexValue for index updated.");
        }
        signer = await ethers.getSigner(publisher);
        if (isDistribute) {
            txnResponse = await ida
        ...
        if (isRevoke == null || sender == null) {
            throw new Error("You must pass isRevoke and sender for subscription revoked.");
        }
        if (isRevoke) {
            signer = await ethers.getSigner(subscriber);
            txnResponse = await ida
...
function getIDAEventDataForValidation(
    ...
    const subscriptionId = getSubscriptionId(subscriber, publisher, token, indexId.toString());
    ...
    if (type === IDAEventType.IndexCreated) {
        ...
        if (
            ...
            throw new Error("newIndexValue, totalUnitsApproved and totalUnitsPending are required for IndexUpdated");
        }
        return {
            ...baseEventData,
            index: { id: indexEntityId },
    ...
    } else if ([IDAEventType.IndexSubscribed, IDAEventType.IndexUnsubscribed].includes(type)) {
    ...
    } else if ([IDAEventType.SubscriptionApproved, IDAEventType.SubscriptionRevoked].includes(type)) {
        ...
        if (extraEventData.distributionDelta == null) {
            throw new Error("distributionDelta is required for IndexDistributionClaimed");
        }
        ...
        if (extraEventData.distributionDelta == null) {
            throw new Error("distributionDelta is required for SubscriptionDistributionClaimed");
        }
        const { userData, ...claimEventSubscriptionData } = baseSubscriptionEventData;
        return {
...
async function fetchIDAEventsAndValidate(
    ...
    const eventDataToValidate = getIDAEventDataForValidation(type, baseParams, extraEventData);
...
async function getExpectedDataForIDA(
    ...
    if (type === IDAEventType.IndexCreated) {
        ...
        return await getExpectedDataForSubscriptionDistributionClaimed(expectedDataParams);
    } else if (type === IDAEventType.SubscriptionRevoked) {
        if (extraExpectedData.isRevoke == null) {
            throw new Error("isRevoke is required for SubscriptionRevoked");
        }
        return await getExpectedDataForRevokeOrDeleteSubscription(expectedDataParams, extraExpectedData.isRevoke);
    }
    ...
    // type === IDAEventType.SubscriptionUnitsUpdated
    return await getExpectedDataForSubscriptionUnitsUpdated(expectedDataParams, extraExpectedData.units);
}
