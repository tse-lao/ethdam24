...
export const beforeSetup = async (tokenAmount: number) => {
    ...
    const fDAI = new ethers.Contract(fDAIx.underlyingToken.address, TestTokenABI) as unknown as TestToken;
    ...
    for (let i = 0; i < signers.length; i++) {
        ...
        await fDAI.connect(signers[0]).mint(signers[i].address, stringBigIntAmount);
        await fDAI.connect(signers[i]).approve(fDAIx.address, stringBigIntAmount);
        await fDAIx
            .upgrade({
...
export const getRandomFlowRate = (max: number) => Math.floor(Math.random() * max) + 1;
...
export const subgraphRequest = async <T>(query: string, variables?: { [key: string]: any }): Promise<T> => {
    try {
        ...
        if (response.errors) {
            throw new Error(
                `Failed call to subgraph with query ${query}, error thrown: ${JSON.stringify(response.errors)}`
            );
        }
...
export const fetchEntityAndEnsureExistence = async <T>(query: string, id: string, entityName: string) => {
    const vars = {
        id,
    };
    const data = await subgraphRequest<{ response: T } & ISubgraphErrors>(query, vars);
...
export const fetchEventAndEnsureExistence = async <T>(query: string, transactionHash: string, eventName: string) => {
    const vars = {
        transactionHash,
    };
...
export const getCurrentTotalAmountStreamed = (
    ...
    return toBN(streamedSoFar).add(toBN(outflowRate).mul(toBN(currentTime).sub(toBN(lastUpdatedAtTime))));
};
...
export const getStreamId = (sender: string, receiver: string, token: string, revisionIndex: string) =>
    [sender, receiver, token, revisionIndex + ".0"].map((x) => x.toLowerCase()).join("-");
...
export const getRevisionIndexId = (sender: string, receiver: string, token: string) =>
...
export const getSubscriptionId = (subscriber: string, publisher: string, token: string, indexId: string) =>
    [subscriber, publisher, token, indexId].map((x) => x.toLowerCase()).join("-");
 ...
 */
export const modifyFlowAndReturnCreatedFlowData = async (data: ITestModifyFlowData) => {
    ...
    let signer = data.liquidator ? await ethers.getSigner(data.liquidator) : await ethers.getSigner(data.sender);
    const baseData = {
...
export const updateFlowOperatorPermissions = async (data: ITestUpdateFlowOperatorData) => {
    ...
    if (data.isFullControl) {
        txnResponse = await data.framework.cfaV1.authorizeFlowOperatorWithFullControl(baseData).exec(signer);
    } else if (data.isFullControlRevoke) {
        txnResponse = await data.framework.cfaV1.revokeFlowOperatorWithFullControl(baseData).exec(signer);
    } else {
