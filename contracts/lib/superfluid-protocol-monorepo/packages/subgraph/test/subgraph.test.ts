...
import { ALLOW_CREATE, FlowActionType, IDAEventType } from "./helpers/constants";
import { testFlowUpdated, testModifyIDA, testUpdateFlowOperatorPermissions } from "./helpers/testers";
...
describe("Subgraph Tests", () => {
    ...
    function updateGlobalObjects(data: IUpdateGlobalObjects) {
        if (data.revisionIndexId && data.updatedStreamData) {
            revisionIndexes[data.revisionIndexId] = Number(data.updatedStreamData.revisionIndex);
            periodRevisionIndexes[data.revisionIndexId] = Number(data.updatedStreamData.periodRevisionIndex);
        }
        ...
        if (data.updatedReceiverATS) {
            accountTokenSnapshots[data.updatedReceiverATS.id] = data.updatedReceiverATS;
        }
        if (data.updatedIndex) {
            indexes[data.updatedIndex.id] = data.updatedIndex;
        }
        if (data.updatedSubscription) {
            subscriptions[data.updatedSubscription.id] = data.updatedSubscription;
        }
        if (data.updatedPublisherATS) {
            accountTokenSnapshots[data.updatedPublisherATS.id] = data.updatedPublisherATS;
        }
        if (data.updatedSubscriberATS) {
            accountTokenSnapshots[data.updatedSubscriberATS.id] = data.updatedSubscriberATS;
        }
        if (data.updatedFlowOperator) {
            flowOperators[data.updatedFlowOperator.id] = data.updatedFlowOperator;
        }
    ...
    async function transferAndUpdate(amount: string, sender: SignerWithAddress, receiver: string) {
        ...
        const senderATSId = getATSId(sender.address.toLowerCase(), daix.address.toLowerCase());
        const senderATS = accountTokenSnapshots[senderATSId];

        if (senderATS) {
            const updatedTransferAmount = toBN(senderATS.totalAmountTransferredUntilUpdatedAt).add(toBN(amount));
            accountTokenSnapshots[senderATSId] = {
                ...senderATS,
                totalAmountTransferredUntilUpdatedAt: updatedTransferAmount.toString(),
            };
        }
        const tokenStats = tokenStatistics[daix.address.toLowerCase()];

        if (tokenStats) {
            const timeDelta = toBN(block.timestamp.toString()).sub(toBN(tokenStats.updatedAtTimestamp));
            const streamedAmountSinceUpdatedAt = toBN(tokenStats.totalOutflowRate).mul(timeDelta);
            const totalAmountStreamedUntilUpdatedAt = toBN(tokenStats.totalAmountStreamedUntilUpdatedAt).add(
            ...
            tokenStatistics[daix.address.toLowerCase()] = {
                ...
                totalAmountStreamedUntilUpdatedAt: totalAmountStreamedUntilUpdatedAt.toString(),
            };
        }
    }
    ...
    function getAccountTokenSnapshotsArray() {
        return Object.values(accountTokenSnapshots).filter((x) => x != undefined) as IAccountTokenSnapshot[];
    }

    function getBaseIDAData(baseParams: ISubscriberDistributionTesterParams, provider: BaseProvider) {
        return {
            framework,
            superToken: daix,
    ...
    before(async () => {
        ...
        let { users, sf, fDAI, fDAIx, totalSupply } = await beforeSetup(100000000);
    ...
    describe("Token Tests", () => {
        ...
        it("Should return the correct data for the regularToken", async () => {
            await fetchTokenAndValidate(dai.address.toLowerCase(), "Fake DAI", "fDAI", true, false, "", 18);
        });
        ...
        it("Should be able to unlist and relist tokens", async () => {
            ...
            // List fDAIx
            txn = await framework.contracts.resolver.connect(deployer).set("supertokens.test.fDAIx", daix.address);
    ...
    describe("ConstantFlowAgreementV1 Tests", () => {
        ...
        it.skip("Should liquidate a stream", async () => {
            ...
            const returnAmount = toBN(balanceOfLiquidator.availableBalance).div(toBN(2));

            // transfer balance back to sender
            await transferAndUpdate(returnAmount.toString(), liquidatorSigner, sender);
        });
    ...
    describe("InstantDistributionAgreement Tests", () => {
        ...
        it("Should return correct data after handling case #0.", async () => {
            ...
            for (let i = 1; i < Math.floor(userAddresses.length / 2); i++) {
                ...
                data = await testModifyIDA({
                    ...getBaseIDAData({ ...baseParams, subscriber: "" }, provider),
        ...
        it("Should return correct data after handling case #1.", async () => {
            const token = daix.address;
            // Testing half the users on the last created index.
            for (let i = userAddresses.length / 2 + 1; i < userAddresses.length; i++) {
                const subscriber = userAddresses[i];
                const baseParams: ISubscriberDistributionTesterParams = {
                    token,
        ...
        it("Should return correct data after non-subscribers get units, claim them and publisher updates their units to 0.", async () => {
            ...
            for (let i = Math.floor(userAddresses.length / 2); i < userAddresses.length; i++) {
                const baseParams: ISubscriberDistributionTesterParams = {
                    ...multiBaseParams,
                    subscriber: userAddresses[i],
                };
            ...
            let data = await testModifyIDA({
                ...getBaseIDAData({ ...multiBaseParams, subscriber: "" }, provider),
                eventType: IDAEventType.IndexUpdated,
                amountOrIndexValue,
                isDistribute: true,
            });
    ...
    describe("Global Check", () => {
        it("There should be at least one Account or AccountTokenSnapshot entities with the zero address", async () => {
            const accountTokenSnapshotIds = await fetchEntityAndEnsureExistence<ILightEntity[]>(
                globalQueries.getAccountTokenSnapshotIds,
                ethers.constants.AddressZero,
                "AccountTokenSnapshot"
            );
            const accounts = await fetchEntityAndEnsureExistence<ILightEntity[]>(
            ...
            if (accountTokenSnapshotIds.length === 0) {
                throw new Error("Invariant broken, no nonzero zero address AccountTokenSnapshot's");
            }

            if (accounts.length === 0) {
                throw new Error("Invariant broken, no nonzero zero address Account's");
            }
        });
        ...
        it("Should properly make queries for all events with a LightEntity", async () => {
            ...
            console.log("getSubscriptionDistributionClaimedEventsLightEntities", data);
