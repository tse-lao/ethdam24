...
export const fetchTokenAndValidate = async (
    ...
    const token = await fetchEntityAndEnsureExistence<IToken>(getToken, address, "Token");
...
export const validateTokenEntity = (
    ...
    expect(subgraphToken.symbol, "SuperToken: symbol error").to.equal(expectedSymbol);
    expect(subgraphToken.isListed, "SuperToken: isListed error").to.equal(expectedIsListed);
    expect(subgraphToken.isNativeAssetSuperToken, "SuperToken: isNativeAssetSuperToken error").to.equal(
        expectedIsNativeAssetSuperToken
    );
    ...
    if (subgraphToken.underlyingAddress === ethers.constants.AddressZero) {
        ...
        expect(subgraphToken.underlyingToken!.id).to.equal(expectedUnderlyingAddress.toLowerCase());
    }
};
