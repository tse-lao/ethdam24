...
export const fetchStreamAndValidate = async (
    ...
    const stream = await fetchEntityAndEnsureExistence<IStream>(getStream, streamId, "Stream");

    validateStreamEntity(stream, expectedStreamedUntilUpdatedAt.toString(), streamId, newFlowRate, newDeposit);
...
const validateAccountReverseLookupsForStream = async (stream: IStream) => {
    const senderAccount = await fetchEntityAndEnsureExistence<IAccount>(getAccount, stream.sender.id, "Account");
    const receiverAccount = await fetchEntityAndEnsureExistence<IAccount>(getAccount, stream.receiver.id, "Account");
...
const validateStreamEntity = (
    ...
    expect(subgraphStream.currentFlowRate, "Stream: currentFlowRate error").to.equal(newFlowRate);
    expect(subgraphStream.deposit, "Stream: deposit error").to.equal(newDeposit);
    expect(subgraphStream.streamedUntilUpdatedAt, "Stream: streamedUntilUpdatedAt error").to.equal(
