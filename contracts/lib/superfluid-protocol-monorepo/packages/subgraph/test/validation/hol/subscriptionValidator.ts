...
import { IIndexSubscription, IEvent, IAccount, IIDAEvents, ILightEntity } from "../../interfaces";
import { getAccount, getSubscription } from "../../queries/holQueries";
import { validateReverseLookup } from "../validators";

const validateAccountReverseLookupsForSubscription = async (subscription: IIndexSubscription) => {
    ...
    validateReverseLookup(subscriptionLightEntity, subscriberAccount.subscriptions);
};

const getSubscriptionEventsMap = (indexSubscription: IIndexSubscription, events: IIDAEvents) => {
...
export const fetchSubscriptionAndValidate = async (
    ...
    const indexSubscription = await fetchEntityAndEnsureExistence<IIndexSubscription>(
        getSubscription,
        expectedSubscription.id,
        "Subscription"
    );

    validateSubscriptionEntity(framework, indexSubscription, expectedSubscription, newIndexValue);

    const eventTypeToDataMap = getSubscriptionEventsMap(indexSubscription, events);
...
export const validateSubscriptionEntity = async (
    ...
    const publisher = ethers.utils.getAddress(subgraphSubscription.index.publisher.id);
    const subscriberAddress = ethers.utils.getAddress(subgraphSubscription.subscriber.id);
    ...
    expect(subgraphSubscription.approved, "Subscription: approved error").to.equal(expectedSubscription.approved);
    expect(subgraphSubscription.units, "Subscription: units error").to.equal(expectedSubscription.units);
    expect(
    ...
    expect(subgraphSubscription.indexValueUntilUpdatedAt, "Subscription: indexValueUntilUpdatedAt error").to.equal(
        expectedSubscription.indexValueUntilUpdatedAt
    );

    // Check subgraph data against web3 data
    expect(subgraphSubscription.approved, "Subscription: approved error").to.equal(approved);
    expect(subgraphSubscription.units, "Subscription: units error").to.equal(units);
    const calcPendingDistribution = approved
        ? "0"
        : toBN(subgraphSubscription.units).mul(
              toBN(newIndexValue).sub(toBN(subgraphSubscription.indexValueUntilUpdatedAt))
          );
