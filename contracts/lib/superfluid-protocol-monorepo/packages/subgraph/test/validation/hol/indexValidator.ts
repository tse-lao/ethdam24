...
import { IIndex, IEvent, IAccount, IIDAEvents, ILightEntity } from "../../interfaces";
import { getAccount, getIndex } from "../../queries/holQueries";
...
const validateAccountReverseLookupsForIndex = async (index: IIndex) => {
    ...
    const publisherAccount = await fetchEntityAndEnsureExistence<IAccount>(getAccount, index.publisher.id, "Account");
...
const getIndexEventsMap = (index: IIndex, events: IIDAEvents) => {
    return new Map<IDAEventType, { event: IEvent | undefined; events: ILightEntity[] | undefined }>([
        [
...
export const fetchIndexAndValidate = async (
    ...
    const index = await fetchEntityAndEnsureExistence<IIndex>(getIndex, expectedIndex.id, "Index");

    validateIndexEntity(framework, index, expectedIndex);
    const eventTypeToDataMap = getIndexEventsMap(index, events);

    if (eventType === IDAEventType.IndexCreated && index.indexCreatedEvent != null) {
        ...
        validateReverseLookup(events.IndexCreatedEvent, [index.indexCreatedEvent]);
    ...
    if (Array.from(eventTypeToDataMap.keys()).includes(eventType)) {
        ...
        if (!data || !data.event || !data.events) {
            throw new Error("You must have all the data for validating reverse lookup.");
        }
    ...
    if (
        ...
        (eventType === IDAEventType.SubscriptionUnitsUpdated || eventType === IDAEventType.SubscriptionApproved)
    ) {
...
export const validateIndexEntity = async (framework: Framework, subgraphIndex: IIndex, expectedIndex: IIndex) => {
    ...
    expect(subgraphIndex.indexId, "Index: indexId error").to.equal(expectedIndex.indexId);
    expect(subgraphIndex.indexValue, "Index: index value error").to.equal(expectedIndex.indexValue);
    expect(subgraphIndex.totalSubscriptionsWithUnits, "Index: totalSubscriptionWithUnits error").to.equal(
        expectedIndex.totalSubscriptionsWithUnits
    );
    expect(subgraphIndex.totalUnitsPending, "Index: totalUnitsPending error").to.equal(expectedIndex.totalUnitsPending);
    expect(subgraphIndex.totalUnitsApproved, "Index: totalUnitsApproved error").to.equal(
        expectedIndex.totalUnitsApproved
    );
    expect(subgraphIndex.totalUnits, "Index: totalUnits error").to.equal(expectedIndex.totalUnits);
    expect(
    ...
    expect(subgraphIndex.indexValue, "Index: index value error").to.equal(indexValue.toString());
    expect(subgraphIndex.totalUnitsApproved, "Index: totalUnitsApproved error").to.equal(totalUnitsApproved.toString());
    expect(subgraphIndex.totalUnitsPending, "Index: totalUnitsPending error").to.equal(totalUnitsPending);
    expect(subgraphIndex.totalUnits, "Index: totalUnits error").to.equal(totalUnits.toString());
};
