...
export async function validateFlowUpdated(
    ...
    await fetchStreamPeriodAndValidate(pastStreamData, newFlowRate.toString(), event, actionType, newDeposit);
...
export async function validateModifyIDA(
    ...
    await fetchIndexAndValidate(framework, updatedIndex, eventType, events, updatedSubscription.id, subscriptionExists);
...
export function validateReverseLookup(entity: ILightEntity, entities: ILightEntity[]) {
