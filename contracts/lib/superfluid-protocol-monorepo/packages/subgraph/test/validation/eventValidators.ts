...
export const fetchEventAndValidate = async <EventType extends IEvent, ExpectedDataType>(
    txnResponse: TransactionResponse,
    expectedData: ExpectedDataType,
    query: string,
    eventName: string
) => {
    const event = await fetchEventAndEnsureExistence<EventType>(query, txnResponse.hash, eventName);
...
export const validateData = <T>(queriedData: T, expectedData: { [key: string]: any }) => {
...
export const validateBaseEventData = (queriedEvent: IEvent, txnResponse: TransactionResponse, queryName: string) => {
