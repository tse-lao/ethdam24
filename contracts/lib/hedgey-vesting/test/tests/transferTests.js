...
module.exports = () => {
  ...
  it('recipient of a plan can toggle on and off the transfer OBO', async () => {
    ...
    expect(await vesting.connect(a).toggleAdminTransferOBO(tokenId, false))
      .to.emit('PlanVestingAdminTransferToggle')
      .withArgs(tokenId, false);
    await expect(vesting.connect(admin).transferFrom(a.address, b.address, tokenId)).to.be.revertedWith(
      '!transferrable'
    );
    expect(await vesting.connect(a).toggleAdminTransferOBO(tokenId, true))
      .to.emit('PlanVestingAdminTransferToggle')
    ...
    tx = await voteVesting.createPlan(
      a.address,
      token.address,
      amount,
      start,
    ...
    expect(await voteVesting.connect(a).toggleAdminTransferOBO(tokenId, false))
      .to.emit('PlanVestingAdminTransferToggle')
      .withArgs(tokenId, false);
    await expect(voteVesting.connect(admin).transferFrom(a.address, b.address, tokenId)).to.be.revertedWith(
      '!transferrable'
    );
    expect(await voteVesting.connect(a).toggleAdminTransferOBO(tokenId, true))
      .to.emit('PlanVestingAdminTransferToggle')
      .withArgs(tokenId, true);
