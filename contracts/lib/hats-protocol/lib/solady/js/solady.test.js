...
function testCompressDecompress(compress, decompress) {
  ...
  for (var t = 0; t < 1000; ++t) {
    ...
    assertEq(
      decompressed.replace(/^0x/, ""),
      data.toLowerCase().replace(/^0x/, ""),
    );
  }
  assert(
    totalCompressedLength < totalDataLength,
    "Compress not working as intended.",
  );
...
test("LibZip: FastLZ compress / decompress.", function () {
  testCompressDecompress(
    solady.LibZip.flzCompress,
    solady.LibZip.flzDecompress,
  );
});
