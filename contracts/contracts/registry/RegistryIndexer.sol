...
abstract contract RegistryIndexer is DomainObjs {
    ...
    string private constant PROFILE_SCHEMA =
        "profileID text, name text, metadata text, profileAddress text";
    ...
    constructor(address _registry) {
        ...
        createTableStatements.push(
            SQLHelpers.toCreateFromSchema(PROFILE_MEMBERS_SCHEMA, PROFILE_MEMBERS_TABLE_PREFIX)
        );

        createTableStatements.push(
            SQLHelpers.toCreateFromSchema(POOLS_MACI_SCHEMA, POOL_MACI_TABLE_PREFIX)
        );
